"""Tests for cursor_scale_v1 policy on Suite 4: linear-classification MIXED suite.

Covers:
  - Invariants (p_total >= 2, p_num >= 1, p_cat >= 1, n >= 5*p_total, p_total <= 30)
  - Budget carve: p_num + p_cat == p_total
  - Regime coverage
  - Audit completeness
  - Determinism
"""
...
def _run(seed: int = BASE_SEED, n_total: int = N_TOTAL) -> tuple[list[dict], dict]:
    ...
    return allocate_mixed_classification_tasks_cursor(
        splits, PROFILE, seed, allocation_mode="weighted_quota", min_regime_count=1
    )


class TestMixedClassificationCursorInvariants:
    ...
    def test_j_regime_never_underdetermined(self, run_result):
        ...
        for a in j_tasks:
            assert a["n"] >= _CURSOR_MIN_N_OVER_P * a["p_total"], (
                f"cursor J mixed task violates n>=5p: n={a['n']}, p={a['p_total']}"
            )


class TestMixedClassificationCursorAudit:
    @pytest.fixture(scope="class")
    def run_result(self):
        return _run()
    ...
    def test_audit_has_dimension_policy(self, run_result):
        ...
        assert dp["p_num_min"] == 1
        assert dp["p_cat_min"] == 1
...
class TestMixedClassificationCursorRegimeCoverage:
    @pytest.fixture(scope="class")
    def run_result(self):
    ...
    def test_ef_softmax_present(self, run_result):
        ...
        assert len(ef) == 2, f"Missing E or F softmax in mixed cls cursor: {ef}"


class TestMixedClassificationCursorDeterminism:
