...
Structural and correctness checks for DeepSetICLModel (MODEL4).
...
Nine checks:
  ...
  3. check_gate_range                      — gate in (0,1) for 10 random contexts (MODEL3 legacy path)
  ...
  7. check_model4_linear_stat_permutation  — stat extractor invariant to row/col permutations
  8. check_model4_coefficient_head_variable_p — coeff head produces (p,) for p=3, p=7, p=50
  9. check_model4_amp_dtype_safety         — bfloat16 autocast; extractor stays float32; y_hat finite
...
def _fresh_model(use_ridge_expert: bool = False, device=None,
...
def check_gate_range(model=None, device=None) -> dict:
    ...
    # Skip for classification models or models without ridge gate
    task_obj = getattr(getattr(model, "cfg", None), "task_objective", "")
    if task_obj in ("inductive_classification", "linear_classification"):
        return {"passed": True, "reason": "classification model; gate check skipped"}
    if not hasattr(model, "gate_head"):
        return {"passed": True, "reason": "no gate_head; check skipped"}
    model.eval()
