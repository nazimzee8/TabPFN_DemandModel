...
dgp_helpers.py
==============
Shared DGP logic for generate_dgp.py and generate_linear_regression.py.
...
# ---------------------------------------------------------------------------
# Cursor-scale dimension policy constants  (linear_stat_aware numeric regression)
# ---------------------------------------------------------------------------
_CURSOR_P_LAMBDA: int = 10            # Poisson lambda — main p_total component
_CURSOR_N_LAMBDA: int = 200           # Poisson lambda — n_total
_CURSOR_TAIL_FRACTION: float = 0.05   # Probability of drawing from wide-p component
_CURSOR_TAIL_P_LAMBDA: int = 25       # Poisson lambda — wide-p tail
_CURSOR_TAIL_P_CAP: int = 30          # Hard cap on tail p_total draws
_CURSOR_TAIL_P_FLOOR: int = 20        # Lower bound on tail p_total draws (truncated Poisson)
_CURSOR_MIN_N_OVER_P: int = 5         # Minimum n_total / p_total

# Seed magic constant for Cursor-scale per-task seeds (regression).
# Must differ from classification (0xC1A55) and mixed (0xCA7E60 / 0xCA7C15).
_REG_CURSOR_MAGIC: int = 0xCA7C0D3

# Seed magic constants for the other 3 linear cursor allocators.
# Each must be unique across all per-task seed derivations in the codebase.
_MIX_REG_CURSOR_MAGIC: int = 0xCA7C1D       # linear-regression mixed
_CLS_CURSOR_MAGIC: int = 0xC1A550C          # linear-classification numeric
_MIX_CLS_CURSOR_MAGIC: int = 0xCA7C1F4     # linear-classification mixed
...
def sample_cursor_dims(rng: np.random.Generator) -> tuple[int, int]:
    """Sample (n_total, p_total) from the Cursor-scale Poisson mixture policy.

    p_total is drawn from a mixture:
      - with prob (1 - _CURSOR_TAIL_FRACTION): Poisson(_CURSOR_P_LAMBDA)
      - with prob _CURSOR_TAIL_FRACTION:        Poisson(_CURSOR_TAIL_P_LAMBDA),
                                                capped at _CURSOR_TAIL_P_CAP
    n_total ~ Poisson(_CURSOR_N_LAMBDA).
    Redraw until p_total >= 1 and n_total >= _CURSOR_MIN_N_OVER_P * p_total.

    Used exclusively by the linear_stat_aware numeric-regression Cursor allocator.
    The legacy profile keeps its own pure-Poisson(10) draw to remain bit-identical.
    """
    while True:
        if rng.random() < _CURSOR_TAIL_FRACTION:
            p = int(max(20, min(int(rng.poisson(_CURSOR_TAIL_P_LAMBDA)), _CURSOR_TAIL_P_CAP)))
        else:
            p = int(rng.poisson(_CURSOR_P_LAMBDA))
        n = int(rng.poisson(_CURSOR_N_LAMBDA))
        if p >= 1 and n >= _CURSOR_MIN_N_OVER_P * p:
            return n, p
...
def _build_E_high_dim_dense(rng: np.random.Generator, params: dict) -> dict:
    if params.get("cursor_dims"):
        # Cursor path: use allocator-provided dims verbatim (no clamp).
        # E is the upper-feature-width Cursor-scale dense regime; dimensions
        # come pre-validated (n >= 5*p) from allocate_regression_tasks_cursor.
        p_signal = int(params["p_signal"])
        n = int(params["n"])
    else:
...
def _build_F_high_dim_sparse(rng: np.random.Generator, params: dict) -> dict:
    if params.get("cursor_dims"):
        # Cursor path: use allocator-provided dims verbatim (no clamp).
        # F is the upper-feature-width Cursor-scale sparse regime.
        p_signal = int(params["p_signal"])
        n = int(params["n"])
        active_s = max(1, min(int(params.get("active_s", max(1, p_signal // 4))), p_signal))
    else:
        p_signal = max(32, min(128, params["p_signal"]))
...
def _build_G_noise_features(rng: np.random.Generator, params: dict) -> dict:
    if params.get("cursor_dims"):
        # Cursor path: allocator guarantees p_signal>=1, p_noise>=1,
        # p_signal+p_noise==p_total, and n>=5*p_total.
        p_signal = int(params["p_signal"])
        p_noise = max(1, int(params.get("p_noise", 1)))
        n = int(params["n"])
        active_s = int(params.get("active_s", p_signal))
    else:
...
def _build_H_block_correlated(rng: np.random.Generator, params: dict) -> dict:
    if params.get("cursor_dims"):
        # Cursor path: use allocator-provided dims verbatim.
        # Clamp block_size to fit within the sampled p_signal.
        p_signal = int(params["p_signal"])
        n = int(params["n"])
        valid_blocks = [b for b in [4, 8, 16] if b <= p_signal]
        block_size = int(rng.choice(valid_blocks)) if valid_blocks else max(1, p_signal // 2)
    else:
...
def _build_J_low_n_high_p(rng: np.random.Generator, params: dict) -> dict:
    if params.get("cursor_dims"):
        # Cursor path: J is the most information-constrained VALID Cursor-scale regime.
        # The allocator selects J tasks from the intersection of:
        #   - upper 20% of realized p_total, and
        #   - lowest 20% of realized n/p ratio.
        # n >= 5*p is guaranteed by the Cursor policy — J is NEVER underdetermined here.
        # The old "n < p" forcing is removed on the cursor path.
        p_signal = int(params["p_signal"])
        n = int(params["n"])
        active_s = max(1, min(int(params.get("active_s", max(1, p_signal // 4))), p_signal))
    else:
        p_signal = max(8, min(64, params["p_signal"]))
        # Legacy path: force n <= p_total (underdetermined) as before.
...
def _build_L_market_linear(rng: np.random.Generator, params: dict) -> dict:
    if params.get("cursor_dims"):
        # Cursor path: use allocator-provided dims verbatim.
        p_signal = int(params["p_signal"])
        p_noise = int(params.get("p_noise", 0))
        n = int(params["n"])
    else:
        p_signal = max(8, min(64, params["p_signal"]))
...
def write_parquet_eval(
    ...
    *,
    ...
    """Write evaluation-suite parquet (generate_linear_regression.py format).
...
def _classification_shape_params(
    ...
    if regime == "B_iid_sparse_logistic":
        ...
        if assignment.get("cursor_dims", False):
            # cursor_scale_v1: dims pre-drawn; E is a post-hoc upper-quartile-p label.
            n = int(assignment.get("n", n))
            p_signal = int(assignment.get("p_signal", p_signal))
        else:
        ...
        if assignment.get("cursor_dims", False):
            # cursor_scale_v1: dims pre-drawn; F is a post-hoc upper-quartile-p label.
            n = int(assignment.get("n", n))
            p_signal = int(assignment.get("p_signal", p_signal))
        else:
            p_signal = max(32, p_signal)
        ...
        pattern = "sparse"
        if assignment.get("cursor_dims", False):
            # cursor_scale_v1: dims pre-drawn by cursor allocator; n >= 5*p guaranteed.
            # Do NOT force n < p or adjust p_noise here — use dims from assignment.
            n = int(assignment.get("n", n))
            p_signal = int(assignment.get("p_signal", p_signal))
            p_noise = int(assignment.get("p_noise", 0))
        else:
            # Legacy grid path: force underdetermined n < p.
...
def generate_classification_dataset(
    ...
    *,
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        # When requested_margin is "low" or "medium" with a strongly imbalanced
        # prior, the intercept calibration (needed to satisfy the target class
        # prior) creates a baseline margin that the simple W-scaling loop cannot
        # correct because scaling W toward zero pushes margin *up* (toward the
        # intercept-dominated high-margin baseline).  This is structurally hard
        # (often infeasible) for:
        #   - "low"    + "severe"   imbalance,  num_classes >= 2
        #   - "low"    + "moderate" imbalance,  num_classes >= 3
        #   - "medium" + "severe"   imbalance,  num_classes >= 5
        # On the second half of attempts, fall back to a balanced prior so the
        # task can still be generated (intercepts ≈ 0 ⇒ W-scaling works cleanly).
        effective_imbalance = assignment["class_imbalance_type"]
        if attempt > max_attempts // 2:
            if requested_margin == "low":
                if effective_imbalance == "severe" and num_classes >= 2:
                    effective_imbalance = "balanced"
                elif effective_imbalance == "moderate" and num_classes >= 3:
                    effective_imbalance = "balanced"
            elif requested_margin == "medium":
                if effective_imbalance == "severe" and num_classes >= 5:
                    effective_imbalance = "balanced"
        target_prior, majority_class = _target_class_prior(
            rng, num_classes, effective_imbalance
        )
        ...
        return {
            ...
            "cursor_dims": bool(assignment.get("cursor_dims", False)),
            "attempt": attempt,
...
def validate_classification_dataset(ds: dict[str, Any]) -> None:
    ...
    if ds["classification_regime"] == "J_low_n_high_p_classification":
        if ds.get("cursor_dims", False):
            # cursor_scale_v1 path: J is a relative-stress post-hoc label;
            # n >= 5*p is ALWAYS guaranteed — n < p is impossible.
            assert n >= _CURSOR_MIN_N_OVER_P * p, (
                f"cursor J task violates n >= 5p: n={n}, p={p}"
            )
        else:
            # Legacy grid path: J_low_n_high_p_classification literally requires n < p.
            assert n < p
...
# ---------------------------------------------------------------------------
# Cursor-scale allocator  (linear_stat_aware numeric regression only)
# ---------------------------------------------------------------------------

def allocate_regression_tasks_cursor(
    ...
    *,
    ...
    feature_noise_grid: "list[float] | None" = None,
    allocation_mode: str = "weighted_quota",
    min_regime_count: int = 1,
    strict_coverage: bool = False,
) -> "tuple[list[dict], dict]":
    """Allocate regression tasks using the Cursor-scale Poisson dimension policy.

    Only supports profile='linear_stat_aware'.

    Dimension policy
    ----------------
    n_total  ~ Poisson(200)
    p_total  ~ 95% Poisson(10),  5% Poisson(25) capped at 30   (wide-p tail)
    Reject until p_total >= 1  and  n_total >= 5 * p_total.

    Regime labelling is performed post-hoc by realized-dimension rank within
    each split — dimensions are NOT redrawn per regime:

      J_low_n_high_p   : upper-20% p_total  AND  lowest-20% n/p ratio.
                         Never underdetermined (n >= 5*p always).
      E_high_dim_dense : upper-25% p_total (from remaining), dense coefficients.
      F_high_dim_sparse: upper-25% p_total (from remaining), sparse coefficients.
      G_noise_features : requires p_total >= 2 (p_signal >= 1, p_noise >= 1).
      A,B,C,D,H,I,K,L : remaining tasks in round-robin quota order.

    Returns (assignments, audit) with assignments ordered:
      first split → second split → third split
    so the positional 80/10/10 outer split in _run_regression is preserved.

    Each assignment dict contains the same keys as allocate_regression_tasks
    plus 'task_seed', 'p_total', and 'cursor_dims=True'.
    """
    if profile != "linear_stat_aware":
        raise ValueError(
            f"allocate_regression_tasks_cursor: only supports "
            f"profile='linear_stat_aware', got {profile!r}"
        )
    ...
    all_weights: dict[str, float] = dict(REGIME_WEIGHTS[profile])  # all 12 regimes, J included
    regimes: list[str] = list(all_weights.keys())

    all_assignments: list[dict] = []
    audit_by_split: dict[str, dict] = {}

    for split_idx, (split_name, n_split) in enumerate(split_sizes.items()):

        # ------------------------------------------------------------------
        # Step 1: Draw per-task Cursor dims + non-dim knobs deterministically
        # ------------------------------------------------------------------
        tasks: list[dict] = []
        for i in range(n_split):
            task_seed = int(
                np.random.SeedSequence(
                    [base_seed, split_idx, i, _REG_CURSOR_MAGIC]
                ).generate_state(1)[0]
            )
            rng = np.random.default_rng(task_seed)
            n_total, p_total = sample_cursor_dims(rng)
            rho = float(rng.choice(rho_grid))
            target_noise_scale = float(rng.choice(target_noise_grid))
            feature_noise_level = float(rng.choice(feature_noise_grid))
            tasks.append({
                ...
                "target_noise_scale": target_noise_scale,
                "feature_noise_level": feature_noise_level,
                "n_over_p": float(n_total) / float(p_total),
            })

        # ------------------------------------------------------------------
        # Step 2: Regime quotas for this split
        # ------------------------------------------------------------------
        if allocation_mode == "weighted_quota":
            regime_counts: dict[str, int] = largest_remainder_counts(all_weights, n_split)
        elif allocation_mode == "balanced":
            n_reg = len(all_weights)
            base_cnt = n_split // n_reg
            regime_counts = {r: base_cnt for r in all_weights}
            rem = n_split - base_cnt * n_reg
            for j, r in enumerate(regimes):
                if j < rem:
                    regime_counts[r] += 1
        else:
            raise ValueError(
                f"allocate_regression_tasks_cursor: unsupported allocation_mode "
                f"{allocation_mode!r}; use 'weighted_quota' or 'balanced'"
            )

        # Enforce min_regime_count per split via convergence loop
        max_iters = 3 * len(regimes) + 1
        for _ in range(max_iters):
            any_deficit = False
            for r in regimes:
                if regime_counts.get(r, 0) < min_regime_count:
                    any_deficit = True
                    deficit = min_regime_count - regime_counts.get(r, 0)
                    sources = [
                        s for s in regimes
                        if regime_counts.get(s, 0) > min_regime_count
                    ]
                    if sources and deficit > 0:
                        try:
                            _move_quota(regime_counts, r, deficit, sources)
                        except ValueError:
                            if strict_coverage:
                                raise ValueError(
                                    f"allocate_regression_tasks_cursor: cannot satisfy "
                                    f"min_regime_count={min_regime_count} for regime "
                                    f"{r!r} in split {split_name!r} (n_split={n_split})"
                                )
            if not any_deficit:
                break

        # ------------------------------------------------------------------
        # Step 3: Rank-based post-hoc label assignment within the split
        # ------------------------------------------------------------------
        n_tasks = len(tasks)

        # Sort indices by p_total descending, breaking ties by task_seed
        p_rank_desc = sorted(
            range(n_tasks),
            key=lambda i: (-tasks[i]["p_total"], tasks[i]["task_seed"]),
        )
        # Sort indices by n/p ascending (lower ratio = more info-constrained)
        ratio_rank_asc = sorted(
            range(n_tasks),
            key=lambda i: (tasks[i]["n_over_p"], tasks[i]["task_seed"]),
        )

        upper_20_p_set = set(p_rank_desc[: max(1, n_tasks // 5)])
        lower_20_ratio_set = set(ratio_rank_asc[: max(1, n_tasks // 5)])

        # Precompute rank positions for joint-score fallback
        p_rank_pos: dict[int, int] = {idx: pos for pos, idx in enumerate(p_rank_desc)}
        ratio_rank_pos: dict[int, int] = {idx: pos for pos, idx in enumerate(ratio_rank_asc)}

        def _j_score(task_idx: int) -> tuple:
            # Lower position in both rank lists is better (higher p, lower n/p)
            return (
                p_rank_pos[task_idx] + ratio_rank_pos[task_idx],
                tasks[task_idx]["task_seed"],
            )

        # J: intersection of upper-20% p and lowest-20% n/p; fill shortfall by joint score
        j_intersection = upper_20_p_set & lower_20_ratio_set
        j_quota = regime_counts.get("J_low_n_high_p", 0)
        j_candidates = sorted(j_intersection, key=_j_score)
        if len(j_candidates) < j_quota:
            all_by_j = sorted(range(n_tasks), key=_j_score)
            extra = [i for i in all_by_j if i not in j_intersection]
            j_candidates = j_candidates + extra
        j_assigned: set[int] = set(j_candidates[:j_quota])

        # E/F: from remaining tasks, upper-25% p_total; first e_quota → E, next → F
        remaining_after_j = [i for i in range(n_tasks) if i not in j_assigned]
        e_quota = regime_counts.get("E_high_dim_dense", 0)
        f_quota = regime_counts.get("F_high_dim_sparse", 0)
        ef_quota = e_quota + f_quota

        ef_candidates = [i for i in p_rank_desc if i in set(remaining_after_j)]
        ef_selected = ef_candidates[:ef_quota]
        e_assigned: set[int] = set(ef_selected[:e_quota])
        f_assigned: set[int] = set(ef_selected[e_quota:ef_quota])
        ef_assigned: set[int] = e_assigned | f_assigned

        remaining_after_ef = [i for i in remaining_after_j if i not in ef_assigned]

        # G: requires p_total >= 2 (p_signal >= 1, p_noise >= 1)
        g_quota = regime_counts.get("G_noise_features", 0)
        g_eligible = [i for i in remaining_after_ef if tasks[i]["p_total"] >= 2]
        g_ineligible_count = len(remaining_after_ef) - len(g_eligible)

        if len(g_eligible) < g_quota:
            # Not enough p_total>=2 tasks for G; log and reduce quota
            actual_g = len(g_eligible)
            shortfall = g_quota - actual_g
            if strict_coverage and shortfall > 0:
                raise ValueError(
                    f"allocate_regression_tasks_cursor: G_noise_features requires p_total>=2 "
                    f"but only {actual_g} eligible tasks in split {split_name!r} "
                    f"(quota={g_quota}); this is extremely rare with Poisson(10) sampling. "
                    f"Reduce min_regime_count or increase n_split."
                )
            g_quota = actual_g
            regime_counts["G_noise_features"] = g_quota
            # Transfer shortfall to A_iid_dense (largest weight, accepts p_total=1)
            regime_counts["A_iid_dense"] = regime_counts.get("A_iid_dense", 0) + shortfall

        g_assigned_indices = g_eligible[:g_quota]
        g_assigned: set[int] = set(g_assigned_indices)

        # Pool of unassigned tasks for A,B,C,D,H,I,K,L
        pool = [i for i in remaining_after_ef if i not in g_assigned]

        # Build the regime → task-index assignment map
        assignment_map: dict[int, str] = {}
        for i in j_assigned:
            assignment_map[i] = "J_low_n_high_p"
        for i in e_assigned:
            assignment_map[i] = "E_high_dim_dense"
        for i in f_assigned:
            assignment_map[i] = "F_high_dim_sparse"
        for i in g_assigned:
            assignment_map[i] = "G_noise_features"

        # Remaining regimes fill from pool sequentially
        other_regimes = [
            r for r in regimes
            if r not in ("J_low_n_high_p", "E_high_dim_dense", "F_high_dim_sparse",
                         "G_noise_features")
        ]
        pool_iter = iter(pool)
        for regime_name in other_regimes:
            count = regime_counts.get(regime_name, 0)
            for _ in range(count):
                try:
                    task_idx = next(pool_iter)
                    assignment_map[task_idx] = regime_name
                except StopIteration:
                    if strict_coverage:
                        raise ValueError(
                            f"allocate_regression_tasks_cursor: pool exhausted for regime "
                            f"{regime_name!r} in split {split_name!r} — "
                            f"quota counts do not sum to n_split={n_split}"
                        )
                    break

        # Assign any remaining pool entries to A_iid_dense (quota accounting drift guard)
        for leftover_idx in pool_iter:
            assignment_map[leftover_idx] = "A_iid_dense"

        # ------------------------------------------------------------------
        # Step 4: Resolve (p_signal, p_noise, active_s, covariance_type) per task
        # ------------------------------------------------------------------
        # Use a deterministic seed offset so this draw does not consume from
        # the same RNG state as the dimension/knob draw above.
        split_assignments: list[dict] = []
        for i, task in enumerate(tasks):
            regime = assignment_map.get(i, "A_iid_dense")
            p_total = task["p_total"]
            rho = task["rho"]
            tns = task["target_noise_scale"]
            fnl = task["feature_noise_level"]

            # Noise features: G always has p_noise>=1; L may optionally
            if regime == "G_noise_features":
                p_noise = max(1, min(p_total - 1, max(1, p_total // 3)))
                p_signal = p_total - p_noise
            elif regime == "L_market_linear":
                # Use a secondary RNG derived from the task_seed to decide noise fraction
                rng_l = np.random.default_rng(task["task_seed"] ^ 0x4C4D4152)
                if p_total >= 2 and rng_l.random() < 0.3:
                    p_noise = max(1, min(p_total - 1, max(1, p_total // 4)))
                    p_signal = p_total - p_noise
                else:
                    p_signal = p_total
                    p_noise = 0
            ...
            # Active support (sparse regimes get p_signal//4)
            if regime in ("B_iid_sparse", "F_high_dim_sparse", "J_low_n_high_p"):
                active_s = max(1, min(max(1, p_signal // 4), p_signal))
            else:
                active_s = p_signal

            # Covariance type — some regimes override rho
            if regime == "D_correlated_ar":
                cov_type = "ar1"
                rho = 0.6   # D always uses rho=0.6 (legacy builder hard-codes this)
            elif regime == "H_block_correlated":
                cov_type = "block"
                rho = 0.7
            elif regime == "I_equicorrelated":
                cov_type = "equicorrelation"
                # rho drawn by _build_I_equicorrelated from its own list
            else:
                cov_type = "ar1" if rho > 0.0 else "iid"

            # K always has non-zero feature noise
            if regime == "K_feature_noise" and fnl == 0.0:
                fnl = 0.1
            ...
            split_assignments.append({
                ...
                "active_s": active_s,
                "rho": rho,
                "target_noise_scale": tns,
                "feature_noise_level": fnl,
                "covariance_type": cov_type,
        ...
        # Per-split audit
        realized_split: dict[str, int] = {}
        for a in split_assignments:
            r = a["prior_regime"]
            realized_split[r] = realized_split.get(r, 0) + 1
        audit_by_split[split_name] = {
            "n_split": n_split,
            "regime_counts": dict(regime_counts),
            "realized_regime_counts": realized_split,
        }

        all_assignments.extend(split_assignments)

    # ------------------------------------------------------------------
    # Build top-level audit with dimension summaries
    # ------------------------------------------------------------------
    realized_all: dict[str, int] = {}
    for a in all_assignments:
        r = a["prior_regime"]
        realized_all[r] = realized_all.get(r, 0) + 1

    p_vals = [a["p_total"] for a in all_assignments]
    n_vals = [a["n"] for a in all_assignments]
    ratio_vals = [float(a["n"]) / float(a["p_total"]) for a in all_assignments]

    def _dim_summary(vals: "list[float]") -> dict:
    ...
    def _by_regime(field: str) -> dict:
        out = {}
        for r in all_weights:
            vals = [a[field] for a in all_assignments if a["prior_regime"] == r]
            if vals:
                out[r] = _dim_summary(vals)
        return out
    ...
    audit: dict = {
        ...
        "cursor_dims": True,
        "dimension_policy": {
            "dimension_policy_name": "cursor_scale_v1",
            "n_distribution": "poisson",
            "n_lambda": _CURSOR_N_LAMBDA,
            "p_distribution": "poisson_with_wide_tail",
            "p_lambda": _CURSOR_P_LAMBDA,
            "p_tail_fraction": _CURSOR_TAIL_FRACTION,
            "p_tail_distribution": "poisson",
            "p_tail_lambda": _CURSOR_TAIL_P_LAMBDA,
            "p_tail_cap": _CURSOR_TAIL_P_CAP,
            "p_tail_floor": _CURSOR_TAIL_P_FLOOR,
            "p_tail_allocation": {
                "E_high_dim_dense": 0.025,
                "F_high_dim_sparse": 0.025,
            },
            "minimum_n_to_p_ratio": float(_CURSOR_MIN_N_OVER_P),
            "dimension_sampling_method": "rejection_sampling",
            "p_total_definition": "total observed features presented to the model (p_signal + p_noise)",
            "regime_specific_dimension_conditions": {
                "E_high_dim_dense": (
                    "upper 25% of realized p_total within split (dense coefficients); "
                    "Cursor-scale relative high-dimensional stress"
                ),
                "F_high_dim_sparse": (
                    "upper 25% of realized p_total within split (sparse coefficients); "
                    "Cursor-scale relative high-dimensional stress"
                ),
                "J_low_n_high_p": (
                    "upper 20% p_total AND lowest 20% n/p ratio within split; "
                    "NEVER underdetermined (n >= 5*p guaranteed); "
                    "Cursor-scale most-information-constrained valid regime"
                ),
                "G_noise_features": (
                    "p_noise >= 1, p_signal >= 1, p_signal + p_noise == p_total"
                ),
            },
            "note_e_f_j": (
                "E, F, and J are Cursor-scale relative-stress regimes after this update. "
                "They do not represent absolute high-dimensional or underdetermined inference. "
                "Dimensions are sampled regime-agnostically from the Cursor Poisson policy; "
                "regime labels are assigned post-hoc by realized-dimension rank."
            ),
        },
        "regime_counts": {
            r: sum(1 for a in all_assignments if a["prior_regime"] == r)
            for r in all_weights
        },
        "realized_regime_counts": realized_all,
        "min_regime_count": min_regime_count,
        "allow_underdetermined": False,
        "realized_p_total_summary": _dim_summary(p_vals),
        "realized_n_summary": _dim_summary(n_vals),
        "realized_n_over_p_summary": _dim_summary(ratio_vals),
        "realized_n_by_regime": _by_regime("n"),
        "realized_p_total_by_regime": _by_regime("p_total"),
        "realized_n_over_p_by_regime": {
            r: _dim_summary([float(a["n"]) / float(a["p_total"])
                             for a in all_assignments if a["prior_regime"] == r])
            for r in all_weights
            if any(a["prior_regime"] == r for a in all_assignments)
        },
        "all_tasks_satisfy_n_ge_5p": all(
            a["n"] >= _CURSOR_MIN_N_OVER_P * a["p_total"] for a in all_assignments
        ),
        "audit_by_split": audit_by_split,
    }

    return all_assignments, audit


def allocate_classification_tasks_cursor(
    split_sizes: "dict[str, int]",
    profile: str,
    base_seed: int,
    *,
    ...
    """cursor_scale_v1 allocator for linear-classification NUMERIC suite.

    Only supports profile='linear_classification_stat_aware'.
    Dims drawn via sample_cursor_dims then regimes assigned post-hoc by rank.
    Tail (5%): 2.5% → E_high_dim_dense_softmax, 2.5% → F_high_dim_sparse_softmax.
    J_low_n_high_p_classification: upper-20% p ∩ lowest-20% n/p, NEVER n<p.
    """
    _CLS_PROFILE = "linear_classification_stat_aware"
    if profile != _CLS_PROFILE:
        raise ValueError(
            f"allocate_classification_tasks_cursor only supports "
            f"profile={_CLS_PROFILE!r}, got {profile!r}"
        )
    ...
    all_weights: dict[str, float] = dict(REGIME_WEIGHTS[profile])
    regimes: list[str] = list(all_weights.keys())
    ...
    for split_idx, (split_name, n_split) in enumerate(split_sizes.items()):
        ...
        for i in range(n_split):
            task_seed = int(
                np.random.SeedSequence(
                    [base_seed, split_idx, i, _CLS_CURSOR_MAGIC]
                ).generate_state(1)[0]
            )
            rng = np.random.default_rng(task_seed)
            n_total, p_total = sample_cursor_dims(rng)
            # p_signal = p_total (no noise cols by default; G_noise handled post-hoc)
            p_signal = p_total
            p_noise = 0
            ...
            rho = float(rng.choice(rho_grid))
            active_s = max(1, min(int(rng.choice(active_s_grid)), p_signal))
        ...
        for _ in range(3 * len(regimes) + 1):
            any_deficit = False
            for r in regimes:
                if regime_counts.get(r, 0) < min_regime_count:
                    any_deficit = True
                    deficit = min_regime_count - regime_counts.get(r, 0)
                    sources = [s for s in regimes if regime_counts.get(s, 0) > min_regime_count]
                    if sources:
                        try:
                            _move_quota(regime_counts, r, deficit, sources)
                        except ValueError:
        ...
        # Post-hoc rank assignment
        n_tasks = len(tasks)
        p_rank_desc = sorted(range(n_tasks), key=lambda i: (-tasks[i]["p_total"], tasks[i]["task_seed"]))
        ratio_rank_asc = sorted(range(n_tasks), key=lambda i: (tasks[i]["n_over_p"], tasks[i]["task_seed"]))

        upper_20_p = set(p_rank_desc[:max(1, n_tasks // 5)])
        lower_20_ratio = set(ratio_rank_asc[:max(1, n_tasks // 5)])
        p_rank_pos = {idx: pos for pos, idx in enumerate(p_rank_desc)}
        ratio_rank_pos = {idx: pos for pos, idx in enumerate(ratio_rank_asc)}

        def _j_score(ti: int) -> tuple:
            return (p_rank_pos[ti] + ratio_rank_pos[ti], tasks[ti]["task_seed"])

        j_quota = regime_counts.get("J_low_n_high_p_classification", 0)
        j_cands = sorted(upper_20_p & lower_20_ratio, key=_j_score)
        if len(j_cands) < j_quota:
            all_by_j = sorted(range(n_tasks), key=_j_score)
            extra = [i for i in all_by_j if i not in set(j_cands)]
            j_cands = j_cands + extra
        j_assigned: set[int] = set(j_cands[:j_quota])

        remaining = [i for i in range(n_tasks) if i not in j_assigned]
        e_quota = regime_counts.get("E_high_dim_dense_softmax", 0)
        f_quota = regime_counts.get("F_high_dim_sparse_softmax", 0)
        ef_quota = e_quota + f_quota
        ef_cands = [i for i in p_rank_desc if i in set(remaining)][:ef_quota]
        e_assigned: set[int] = set(ef_cands[:e_quota])
        f_assigned: set[int] = set(ef_cands[e_quota:])
        ef_assigned = e_assigned | f_assigned

        remaining2 = [i for i in remaining if i not in ef_assigned]
        g_quota = regime_counts.get("G_noise_features_classification", 0)
        g_elig = [i for i in remaining2 if tasks[i]["p_total"] >= 2]
        if len(g_elig) < g_quota:
            shortfall = g_quota - len(g_elig)
            regime_counts["G_noise_features_classification"] = len(g_elig)
            regime_counts["A_iid_dense_logistic"] = regime_counts.get("A_iid_dense_logistic", 0) + shortfall
            g_quota = len(g_elig)
        g_assigned: set[int] = set(g_elig[:g_quota])

        pool = [i for i in remaining2 if i not in g_assigned]
        assignment_map: dict[int, str] = {}
        for i in j_assigned:
            assignment_map[i] = "J_low_n_high_p_classification"
        for i in e_assigned:
            assignment_map[i] = "E_high_dim_dense_softmax"
        for i in f_assigned:
            assignment_map[i] = "F_high_dim_sparse_softmax"
        for i in g_assigned:
            assignment_map[i] = "G_noise_features_classification"

        other = [r for r in regimes if r not in ("J_low_n_high_p_classification", "E_high_dim_dense_softmax", "F_high_dim_sparse_softmax", "G_noise_features_classification")]
        pool_iter = iter(pool)
        for rn in other:
            for _ in range(regime_counts.get(rn, 0)):
                try:
                    assignment_map[next(pool_iter)] = rn
                except StopIteration:
                    break
        for leftover in pool_iter:
            assignment_map[leftover] = "A_iid_dense_logistic"

        # Resolve per-task details
        split_assignments: list[dict] = []
        for i, task in enumerate(tasks):
            regime = assignment_map.get(i, "A_iid_dense_logistic")
            p_signal = task["p_signal"]
            p_noise = task["p_noise"]
            rho = task["rho"]
            active_s = task["active_s"]

            if regime == "G_noise_features_classification" and p_signal >= 2:
                p_noise = max(1, min(p_signal - 1, max(1, p_signal // 3)))

            if regime in ("B_iid_sparse_logistic", "F_high_dim_sparse_softmax", "J_low_n_high_p_classification"):
                active_s = max(1, min(active_s, max(1, p_signal // 4)))

            if regime == "D_correlated_ar_logistic":
                cov_type, rho = "ar1", max(rho, 0.3)
            elif regime == "H_block_correlated_classification":
                cov_type, rho = "block", 0.7
            elif regime == "I_equicorrelated_classification":
                cov_type = "equicorrelation"
                rho = max(rho, 0.3)
            else:
            ...
            split_assignments.append({
                ...
                "label_noise_rate": task["label_noise_rate"],
                "rho": rho,
                "active_s": active_s,
                "covariance_type": cov_type,
                "task_seed": task["task_seed"],
        ...
        realized_split: dict[str, int] = {}
        for a in split_assignments:
            realized_split[a["prior_regime"]] = realized_split.get(a["prior_regime"], 0) + 1
        audit_by_split[split_name] = {
            "n_split": n_split,
            "regime_counts": dict(regime_counts),
            "realized_regime_counts": realized_split,
    ...
    realized_all: dict[str, int] = {}
    for a in all_assignments:
        realized_all[a["prior_regime"]] = realized_all.get(a["prior_regime"], 0) + 1

    def _dim_summary(vals: list) -> dict:
        arr = np.asarray(vals, dtype=float)
        return {"mean": float(arr.mean()), "min": float(arr.min()), "p25": float(np.percentile(arr, 25)),
    ...
    audit: dict = {
        ...
        "dimension_policy": {
            ...
            "p_tail_floor": _CURSOR_TAIL_P_FLOOR,
            "minimum_n_to_p_ratio": float(_CURSOR_MIN_N_OVER_P),
            "p_total_definition": "p_signal + p_noise",
            "p_tail_allocation": {
                "E_high_dim_dense_softmax": 0.025,
                "F_high_dim_sparse_softmax": 0.025,
            },
        },
        "regime_counts": {r: sum(1 for a in all_assignments if a["prior_regime"] == r) for r in all_weights},
        "realized_regime_counts": realized_all,
        "min_regime_count": min_regime_count,
        "realized_p_total_summary": _dim_summary(p_vals),
        "realized_n_summary": _dim_summary(n_vals),
        "realized_n_over_p_summary": _dim_summary(ratio_vals),
        "realized_n_by_regime": {r: _dim_summary([a["n"] for a in all_assignments if a["prior_regime"] == r]) for r in all_weights if any(a["prior_regime"] == r for a in all_assignments)},
        "realized_p_total_by_regime": {r: _dim_summary([a["p_total"] for a in all_assignments if a["prior_regime"] == r]) for r in all_weights if any(a["prior_regime"] == r for a in all_assignments)},
        "all_tasks_satisfy_n_ge_5p": all(a["n"] >= _CURSOR_MIN_N_OVER_P * a["p_total"] for a in all_assignments),
        "audit_by_split": audit_by_split,
    }
    return all_assignments, audit
...
def build_mixed_classification_dataset(
    ...
    # Reserve a distinct row for EVERY class: first anchor one existing row per
    # already-present class, then patch missing classes into the best-scoring
    # unreserved row. This prevents patching one class from overwriting the sole
    # occurrence of a class that was already satisfied earlier in the loop.
    # (The original code only reserved patched rows, so a later patch could steal
    # the singleton row of an already-present class, making it absent.)
    if n >= K:
        reserved: set = set()
        # Anchor one row for each class already present, so it cannot be overwritten.
        for k in range(K):
            present_rows = np.flatnonzero(y == k)
            if present_rows.size > 0:
                reserved.add(int(present_rows[0]))
        # Patch each missing class into its best-scoring unreserved row.
        for k in range(K):
            if not np.any(y == k):
                scores = probs[:, k].copy()
...
def allocate_mixed_regression_tasks_cursor(
    split_sizes: "dict[str, int]",
    profile: str,
    base_seed: int,
    *,
    ...
    """cursor_scale_v1 allocator for linear-regression MIXED suite.

    Only supports profile='linear_regression_mixed_categorical_stat_aware'.
    p_total = p_num + p_cat, both >= 1.  Same Poisson base as LR numeric.
    Tail tasks (wide-p) are distributed 2.5% E_high_dim_dense / 2.5% F_high_dim_sparse
    by the same post-hoc rank mechanism as allocate_regression_tasks_cursor.
    """
    _MIXED_PROFILE = "linear_regression_mixed_categorical_stat_aware"
    if profile != _MIXED_PROFILE:
        raise ValueError(
            f"allocate_mixed_regression_tasks_cursor only supports "
            f"profile={_MIXED_PROFILE!r}, got {profile!r}"
        )
    ...
    _P_CAT_SIGNAL_OPTIONS: list[int] = [1, 2, 3]
    _P_CAT_NOISE_OPTIONS: list[int] = [0, 2, 4, 8]
    ...
    for split_idx, (split_name, n_split) in enumerate(split_sizes.items()):
        ...
        # Step 1: Draw per-task dims
        tasks: list[dict] = []
        for i in range(n_split):
            task_seed = int(
                np.random.SeedSequence(
                    [base_seed, split_idx, i, _MIX_REG_CURSOR_MAGIC]
                ).generate_state(1)[0]
            )
            rng = np.random.default_rng(task_seed)

            # Need p_total >= 2 for mixed (p_num >= 1, p_cat >= 1)
            while True:
                n_total, p_total = sample_cursor_dims(rng)
                if p_total >= 2:
                    break

            # Carve p_cat from p_total
            p_cat_signal = min(int(rng.choice(_P_CAT_SIGNAL_OPTIONS)), p_total - 1)
            p_cat_signal = max(p_cat_signal, 1)
            p_num = p_total - p_cat_signal  # p_num >= 1

            # Categorical noise columns: clamp so total stays <= 30 and n >= 5*p
            p_cat_noise = int(rng.choice(_P_CAT_NOISE_OPTIONS))
            max_noise = _CURSOR_TAIL_P_CAP - p_total
            p_cat_noise = max(0, min(p_cat_noise, max_noise))
            p_cat = p_cat_signal + p_cat_noise
            p_total_final = p_num + p_cat
            if n_total < _CURSOR_MIN_N_OVER_P * p_total_final:
                p_cat_noise = 0
                p_cat = p_cat_signal
                p_total_final = p_num + p_cat
            ...
            tasks.append({
                ...
                "target_noise_scale": target_noise_scale,
                "feature_noise_level": feature_noise_level,
                "n_over_p": float(n_total) / float(p_total_final),
            })

        # Step 2: Regime quotas
        if allocation_mode == "weighted_quota":
            regime_counts: dict[str, int] = largest_remainder_counts(all_weights, n_split)
        else:
            n_reg = len(all_weights)
        ...
        # Step 3: Post-hoc rank-based regime assignment
        n_tasks = len(tasks)
        p_rank_desc = sorted(range(n_tasks), key=lambda i: (-tasks[i]["p_total"], tasks[i]["task_seed"]))
        ratio_rank_asc = sorted(range(n_tasks), key=lambda i: (tasks[i]["n_over_p"], tasks[i]["task_seed"]))
        ...
        j_quota = regime_counts.get("J_low_n_high_p", 0)
        j_candidates = sorted(upper_20_p & lower_20_ratio, key=_j_score)
        if len(j_candidates) < j_quota:
            extra = [i for i in sorted(range(n_tasks), key=_j_score) if i not in set(j_candidates)]
            j_candidates = j_candidates + extra
        j_assigned: set[int] = set(j_candidates[:j_quota])
        ...
        if len(g_elig) < g_quota:
            shortfall = g_quota - len(g_elig)
            regime_counts["G_noise_features"] = len(g_elig)
            regime_counts["A_iid_dense"] = regime_counts.get("A_iid_dense", 0) + shortfall
            g_quota = len(g_elig)
        g_assigned: set[int] = set(g_elig[:g_quota])
        ...
        other_regimes = [r for r in regimes if r not in ("J_low_n_high_p", "E_high_dim_dense", "F_high_dim_sparse", "G_noise_features")]
        pool_iter = iter(pool)
        for regime_name in other_regimes:
            for _ in range(regime_counts.get(regime_name, 0)):
                try:
                    assignment_map[next(pool_iter)] = regime_name
                except StopIteration:
                    break
        for leftover in pool_iter:
            assignment_map[leftover] = "A_iid_dense"

        # Step 4: Build assignment dicts
        split_assignments: list[dict] = []
        for i, task in enumerate(tasks):
            regime = assignment_map.get(i, "A_iid_dense")
            p_num = task["p_num"]
            ...
            if regime == "G_noise_features" and p_num >= 2:
                p_num_noise = max(1, min(p_num - 1, max(1, p_num // 3)))
                p_num_signal = p_num - p_num_noise
            elif regime == "L_market_linear":
                _l_rng = np.random.default_rng(task["task_seed"] ^ 0x4C4D4152)
                if p_num >= 2 and _l_rng.random() < 0.3:
                    p_num_noise = max(1, min(p_num - 1, max(1, p_num // 4)))
                    p_num_signal = p_num - p_num_noise
                else:
                    p_num_signal, p_num_noise = p_num, 0
            else:
                p_num_signal, p_num_noise = p_num, 0

            if regime in ("B_iid_sparse", "F_high_dim_sparse", "J_low_n_high_p"):
                active_s = max(1, min(max(1, p_num_signal // 4), p_num_signal))
            else:
                active_s = p_num_signal

            if regime == "D_correlated_ar":
                cov_type, rho = "ar1", 0.6
            elif regime == "H_block_correlated":
                cov_type, rho = "block", 0.7
            elif regime == "I_equicorrelated":
                cov_type = "equicorrelation"
            ...
            split_assignments.append({
                ...
                "p_num_signal": p_num_signal,
                "p_num_noise": p_num_noise,
                "p_cat_signal": task["p_cat_signal"],
                "p_cat_noise": task["p_cat_noise"],
                "p_total": task["p_total"],
                "active_s": active_s,
                "cardinalities": task["cardinalities"],
                "cat_effect_scale": task["cat_effect_scale"],
                "missing_rate": task["missing_rate"],
                "imbalance_type": task["imbalance_type"],
                "rho": rho,
                "target_noise_scale": task["target_noise_scale"],
                "feature_noise_level": fnl,
                "covariance_type": cov_type,
                "task_seed": task["task_seed"],
                "cursor_dims": True,
    ...
    p_vals = [a["p_total"] for a in all_assignments]
    n_vals = [a["n"] for a in all_assignments]
    p_num_vals = [a["p_num"] for a in all_assignments]
    p_cat_vals = [a["p_cat"] for a in all_assignments]
    ratio_vals = [float(a["n"]) / float(a["p_total"]) for a in all_assignments]
    ...
    audit: dict = {
        ...
        "dimension_policy": {
            ...
            "p_tail_floor": _CURSOR_TAIL_P_FLOOR,
            "minimum_n_to_p_ratio": float(_CURSOR_MIN_N_OVER_P),
            "p_total_definition": "p_num + p_cat (categorical carved from same Poisson budget)",
            "p_total_min": 2,
            "p_num_min": 1,
            "p_cat_min": 1,
            "wide_p_tail_allocation": {
                "E_high_dim_dense": "2.5% of all tasks (post-hoc upper-quartile p_total, dense)",
                "F_high_dim_sparse": "2.5% of all tasks (post-hoc upper-quartile p_total, sparse)",
            },
        },
        "regime_counts": {r: sum(1 for a in all_assignments if a["prior_regime"] == r) for r in all_weights},
        "realized_regime_counts": realized_all,
        ...
        "realized_p_total_summary": _dim_summary(p_vals),
        "realized_n_summary": _dim_summary(n_vals),
        "realized_p_num_summary": _dim_summary(p_num_vals),
        "realized_p_cat_summary": _dim_summary(p_cat_vals),
        "realized_n_over_p_summary": _dim_summary(ratio_vals),
        "realized_n_by_regime": {r: _dim_summary([a["n"] for a in all_assignments if a["prior_regime"] == r]) for r in all_weights if any(a["prior_regime"] == r for a in all_assignments)},
        "realized_p_total_by_regime": {r: _dim_summary([a["p_total"] for a in all_assignments if a["prior_regime"] == r]) for r in all_weights if any(a["prior_regime"] == r for a in all_assignments)},
        "all_tasks_satisfy_n_ge_5p": all(a["n"] >= _CURSOR_MIN_N_OVER_P * a["p_total"] for a in all_assignments),
        "all_tasks_satisfy_p_total_ge_2": all(a["p_total"] >= 2 for a in all_assignments),
        "all_tasks_satisfy_p_num_ge_1": all(a["p_num"] >= 1 for a in all_assignments),
        "all_tasks_satisfy_p_cat_ge_1": all(a["p_cat"] >= 1 for a in all_assignments),
        "audit_by_split": audit_by_split,
    }
    return all_assignments, audit
...
def allocate_mixed_classification_tasks_cursor(
    split_sizes: "dict[str, int]",
    profile: str,
    base_seed: int,
    *,
    ...
    """cursor_scale_v1 allocator for linear-classification MIXED suite.

    Only supports profile='linear_classification_mixed_categorical_stat_aware'.
    p_total = p_num + p_cat, both >= 1.
    Tail: 2.5% E_high_dim_dense_softmax / 2.5% F_high_dim_sparse_softmax (post-hoc rank).
    """
    _MIX_CLS_PROFILE = "linear_classification_mixed_categorical_stat_aware"
    if profile != _MIX_CLS_PROFILE:
        raise ValueError(
            f"allocate_mixed_classification_tasks_cursor only supports "
            f"profile={_MIX_CLS_PROFILE!r}, got {profile!r}"
        )
    ...
    for split_idx, (split_name, n_split) in enumerate(split_sizes.items()):
        ...
        for i in range(n_split):
            task_seed = int(
                np.random.SeedSequence(
                    [base_seed, split_idx, i, _MIX_CLS_CURSOR_MAGIC]
                ).generate_state(1)[0]
            )
            rng = np.random.default_rng(task_seed)
            ...
            p_cat = min(int(rng.integers(1, 4)), p_total - 1)
            p_cat = max(p_cat, 1)
            p_num = p_total - p_cat

            p_cat_signal = min(int(rng.choice([1, 2, 4, 8])), p_cat)
            p_cat_signal = max(p_cat_signal, 1)
            p_cat_noise = p_cat - p_cat_signal
        ...
        for i, task in enumerate(tasks):
            ...
            if regime in ("B_iid_sparse_logistic", "F_high_dim_sparse_softmax", "J_low_n_high_p_classification"):
                active_s = max(1, min(p_num // 4, p_num))
            else:
                active_s = p_num
            ...
            if regime == "D_correlated_ar_logistic":
                ...
                cov_type, rho = "equicorrelation", max(rho, 0.3)
            else:
                cov_type = "ar1" if rho > 0.0 else "iid"
