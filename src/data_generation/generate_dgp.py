...
from dgp_helpers import (
    ...
    allocate_mixed_classification_tasks_cursor,
...
def _split_directories(out_dir: str, subdir: str = "") -> tuple[str, str, str]:
    ...
    This matches the Snowflake stage convention:
        @META_DATASET_STAGE/{linear|nonlinear}/{regression|classification}/numeric/{split}/  (numeric families)
        @META_DATASET_STAGE/{linear|nonlinear}/{regression|classification}/mixed/{split}/    (mixed-categorical families)
    """
...
def _write_regression_manifest(
    ...
    generation_command: list,
    allocation_audit: dict,
    task_records: "list[dict]",
    split_counts: "dict[str, int]",
) -> None:
    """Write manifest.json to <out_dir>/numeric/manifest.json for the regression suite."""
    files = sorted(numeric_dir.rglob("*.parquet"))
    checksums = {
        path.relative_to(numeric_dir).as_posix(): _sha256_file(path)
        for path in files
    }
    aggregate = hashlib.sha256()
    for rel_path, cksum in checksums.items():
        aggregate.update(rel_path.encode("utf-8"))
        aggregate.update(b"\0")
        aggregate.update(cksum.encode("ascii"))
        aggregate.update(b"\n")
    output_checksum = aggregate.hexdigest()

    # Realized regime counts by split
    realized_by_split: dict[str, dict[str, int]] = {}
    for rec in task_records:
        s = rec["split"]
        r = rec["prior_regime"]
        realized_by_split.setdefault(s, {})
        realized_by_split[s][r] = realized_by_split[s].get(r, 0) + 1

    suffix_input = (
        f"linear_regression|{args.profile}|linear_regression_v1|"
    ...
    manifest: dict = {
        "suite_id": (
            f"linear_regression-{args.profile}-"
            f"linear_regression_v1-{args.base_seed}-{suffix}"
        ),
        ...
        "schema_version": "linear_regression_v1",
        "store_beta": getattr(args, "store_beta", False),
        "store_teacher_preds": getattr(args, "store_teacher_preds", False),
        "allow_underdetermined": getattr(args, "allow_underdetermined", False),
        "configured_profile_weights": REGIME_WEIGHTS.get(args.profile, {}),
        "allocation": allocation_audit,
        "realized_regime_counts": allocation_audit.get("realized_regime_counts", {}),
        "realized_regime_counts_by_split": realized_by_split,
        "file_checksums": checksums,
        "output_checksum": output_checksum,
        "created_at": dt.datetime.now(dt.timezone.utc).isoformat(),
    }

    # Embed the dimension-policy block from the audit (present for cursor profiles)
    if "dimension_policy" in allocation_audit:
        manifest["dimension_policy"] = allocation_audit["dimension_policy"]
        manifest["realized_p_total_summary"] = allocation_audit.get(
            "realized_p_total_summary", {}
        )
        manifest["realized_n_summary"] = allocation_audit.get("realized_n_summary", {})
        manifest["realized_n_over_p_summary"] = allocation_audit.get(
            "realized_n_over_p_summary", {}
        )
        manifest["realized_n_by_regime"] = allocation_audit.get("realized_n_by_regime", {})
        manifest["realized_p_total_by_regime"] = allocation_audit.get(
            "realized_p_total_by_regime", {}
        )
        manifest["realized_n_over_p_by_regime"] = allocation_audit.get(
            "realized_n_over_p_by_regime", {}
        )
        manifest["all_tasks_satisfy_n_ge_5p"] = allocation_audit.get(
            "all_tasks_satisfy_n_ge_5p", None
        )
        manifest["audit_by_split"] = allocation_audit.get("audit_by_split", {})

    (numeric_dir / "manifest.json").write_text(
...
def _run_regression(args: argparse.Namespace) -> None:
    ...
    # -----------------------------------------------------------------------
    # Choose allocator: Cursor-scale for linear_stat_aware, legacy for others
    # -----------------------------------------------------------------------
    is_cursor = (profile == "linear_stat_aware")
    ...
    if is_cursor:
        ...
        assignments, allocation_audit = allocate_regression_tasks_cursor(
            split_sizes,
            profile,
            base_seed,
            rho_grid=rho_grid,
            ...
            # min_regime_count=1 per split guarantees all 12 regimes appear in each split
            min_regime_count=getattr(args, "min_regime_count", 1),
            strict_coverage=getattr(args, "strict_coverage", False),
        )
        # Cursor allocator provides per-task seeds; no global sequential RNG needed
        rng_global = None
    else:
        ...
        rng_global = np.random.default_rng(seed=base_seed)

    regimes = list(dict(REGIME_WEIGHTS[profile]).keys())
    print(f"Profile: {profile}  ({'cursor-scale Poisson' if is_cursor else 'grid-based'})")
    ...
    for idx, assignment in enumerate(assignments):
        ...
        # Build params dict — include cursor_dims flag when present
        param_keys = [
            "n", "p_signal", "p_noise", "p_total", "active_s", "rho",
            "target_noise_scale", "feature_noise_level", "covariance_type",
            "cursor_dims",
        ]
        params = {k: assignment[k] for k in param_keys if k in assignment}

        # Select RNG: per-task deterministic seed for cursor, global sequential otherwise
        if is_cursor:
            task_rng = np.random.default_rng(assignment["task_seed"])
        else:
            task_rng = rng_global  # type: ignore[assignment]

        ds = build_dataset_from_regime(task_rng, regime, profile, params)
        validate_dataset(ds)
        ...
        # Track per-task record for manifest
        split_label = (
            "train" if idx < n_train_split
            else "val" if idx < n_train_split + n_val_split
            else "test"
        )
        task_records.append({
            "prior_regime": regime,
    ...
    # Write manifest (always for regression; includes dimension-policy block for cursor)
    numeric_dir = Path(out_dir) / "numeric"
    split_counts = {
        "train": n_train_split,
        "val": n_val_split,
        "test": n_test_split,
    }
    _write_regression_manifest(
        numeric_dir,
        args,
...
def _validate_classification_root(out_dir: str) -> Path:
    root = Path(out_dir)
    # Only block if the numeric/ subdir already exists and contains files.
    # A non-empty root is allowed so that a sibling subdir (e.g. mixed/) can
    # coexist without requiring the entire parent to be wiped first.
    numeric_sub = root / "numeric"
    if numeric_sub.exists() and any(numeric_sub.iterdir()):
        raise ValueError(
            "linear_classification requires the numeric/ subdir to be absent or "
            f"empty before generation; {numeric_sub} already contains files. "
            "Remove it first (the runner does this automatically)."
        )
    return root
...
def _run_classification(
    ...
    # -----------------------------------------------------------------------
    # Choose allocator: Cursor-scale for linear_classification_stat_aware, legacy for others
    # -----------------------------------------------------------------------
    is_cursor = (args.profile == "linear_classification_stat_aware")
    ...
    if is_cursor:
        ...
        assignments, allocation_audit = allocate_classification_tasks_cursor(
            split_sizes,
            args.profile,
            args.base_seed,
            num_classes_grid=grids["num_classes_grid"],
            rho_grid=grids["rho_grid"],
            allocation_mode=getattr(args, "allocation_mode", "weighted_quota"),
            min_regime_count=getattr(args, "min_regime_count", 1),
            strict_coverage=getattr(args, "strict_coverage", False),
        )
    ...
    records: list[dict] = []
    print(f"Profile: {args.profile}  ({'cursor-scale Poisson' if is_cursor else 'grid-based'})")
    print(f"Generating {args.n_datasets} classification datasets -> {root}")
    for idx, assignment in enumerate(assignments):
        if is_cursor:
            task_seed = int(assignment["task_seed"])
        else:
            task_seed = int(
                np.random.SeedSequence([args.base_seed, idx, 0xC1A55]).generate_state(1)[0]
            )
        rng = np.random.default_rng(task_seed)
        # Cursor allocator uses "prior_regime"; legacy uses "classification_regime".
        regime = assignment.get("prior_regime") or assignment["classification_regime"]
        if is_cursor and "classification_regime" not in assignment:
            # Normalize cursor assignment dict to satisfy generate_classification_dataset
            assignment = dict(assignment)
            assignment["classification_regime"] = regime
            _meta_rng = np.random.default_rng(task_seed ^ 0x4D455441)
            assignment.setdefault(
                "class_imbalance_type",
                str(_meta_rng.choice(grids["class_imbalance_grid"])),
            )
            assignment.setdefault(
                "margin_level",
                str(_meta_rng.choice(grids["margin_grid"])),
            )
...
def _run_mixed_regression(args: argparse.Namespace) -> None:
    ...
    # -----------------------------------------------------------------------
    # Choose allocator: Cursor-scale for stat_aware, legacy for others
    # -----------------------------------------------------------------------
    _MIX_REG_CURSOR_PROFILE = "linear_regression_mixed_categorical_stat_aware"
    is_cursor = (profile == _MIX_REG_CURSOR_PROFILE)

    if is_cursor:
        _mix_n_train = int(0.8 * n_datasets)
        _mix_n_val = int(0.1 * n_datasets)
        split_sizes = {
            "train": _mix_n_train,
            "val": _mix_n_val,
            "test": n_datasets - _mix_n_train - _mix_n_val,
        }
        assignments, allocation_audit = allocate_mixed_regression_tasks_cursor(
            split_sizes,
            profile,
            base_seed,
...
def _run_mixed_classification(args: argparse.Namespace) -> None:
    ...
    # Choose allocator: Cursor-scale for stat_aware, legacy for others
    # -----------------------------------------------------------------------
    _MIX_CLS_CURSOR_PROFILE = "linear_classification_mixed_categorical_stat_aware"
    is_cursor = (profile == _MIX_CLS_CURSOR_PROFILE)

    if is_cursor:
        _mcls_n_train = int(0.8 * n_datasets)
        _mcls_n_val = int(0.1 * n_datasets)
        split_sizes = {
            "train": _mcls_n_train,
            "val": _mcls_n_val,
            "test": n_datasets - _mcls_n_train - _mcls_n_val,
        }
        assignments, allocation_audit = allocate_mixed_classification_tasks_cursor(
            split_sizes,
            profile,
            base_seed,
            cat_cardinality_grid=parse_int_grid(args.cat_cardinality_grid),
