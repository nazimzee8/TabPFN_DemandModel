"""Build META_MIXED_REGRESSION_DATASET_INDEX from staged mixed-categorical parquet files.

Same pattern as build_meta_dataset_index.py but for mixed-categorical regression tasks.
Reads from @META_DATASET_STAGE/linear/regression/mixed/{split}/ and writes to
META_MIXED_REGRESSION_DATASET_INDEX.
"""
...
_DATA_SUBDIR = "linear/regression/mixed"
INDEX_TABLE = "META_MIXED_REGRESSION_DATASET_INDEX"
SPLITS = ("train", "val", "test")
HPO_BUCKETS = 40
LOCAL_ROOT = "/tmp/meta_mixed_regression_dataset_index"


def _expected_total() -> int:
    return int(os.getenv("META_MIXED_REGRESSION_DATASET_EXPECTED_TOTAL", "1000"))
...
def build_index(session=None, expected_total: int | None = None) -> str:
    """Build META_MIXED_REGRESSION_DATASET_INDEX."""
    if session is None:
    ...
    if len(_live_cols) != _EXPECTED_COL_COUNT:
        raise RuntimeError(
            ...
            "sql/linear_regression_mixed_pipeline.sql before running this script."
        )
    ...
    return f"Indexed {rows_inserted} mixed-regression tasks across {SPLITS}."
