...
under @META_DATASET_STAGE/linear/regression/numeric/{train,val,test}/, reads scalar
...
_DATA_SUBDIR = "linear/regression/numeric"
...
def _write_index(session, rows):
    ...
    live_cols = session.sql(f"DESCRIBE TABLE {META_DATASET_INDEX}").collect()
    live_count = len(live_cols)
    if live_count != len(schema.fields):
        raise RuntimeError(
            f"{META_DATASET_INDEX} has {live_count} columns but writer expects "
            f"{len(schema.fields)} — re-deploy the DDL in "
            "sql/linear_regression_numeric_pipeline.sql before running this script."
        )
