...
classification tasks. Reads from @META_DATASET_STAGE/linear/classification/mixed/{split}/
...
_DATA_SUBDIR = "linear/classification/mixed"
...
def build_index(session=None, expected_total: int | None = None) -> str:
    ...
    # The writer inserts 20 explicit columns; any DDL change must be reflected here.
    ...
    if len(_live_cols) != _EXPECTED_COL_COUNT:
        raise RuntimeError(
            ...
            "sql/linear_classification_mixed_pipeline.sql before running this script."
        )
    ...
    for split in SPLITS:
        ...
        for stage_name in files:
            ...
            session.sql(
                ...
                "prior_regime, hpo_bucket, num_classes, "
