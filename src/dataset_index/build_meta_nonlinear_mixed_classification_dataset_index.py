"""Build META_NONLINEAR_MIXED_CATEGORICAL_DATASET_INDEX from staged nonlinear
mixed-categorical classification parquet files.

Same pattern as build_meta_mixed_classification_dataset_index.py but for nonlinear
mixed-categorical classification tasks. Reads from
@META_DATASET_STAGE/nonlinear/classification/mixed/{split}/ and writes to
META_NONLINEAR_MIXED_CATEGORICAL_DATASET_INDEX.
"""
...
_DATA_SUBDIR = "nonlinear/classification/mixed"
INDEX_TABLE = "META_NONLINEAR_MIXED_CATEGORICAL_DATASET_INDEX"
SPLITS = ("train", "val", "test")
HPO_BUCKETS = 40
LOCAL_ROOT = "/tmp/meta_nonlinear_mixed_classification_dataset_index"


def _expected_total() -> int:
    return int(os.getenv("META_NONLINEAR_MIXED_CLASSIFICATION_DATASET_EXPECTED_TOTAL", "1000"))
...
def _read_metadata(local_path: str) -> dict:
    ...
    return {
        ...
        "classification_regime": str(_get("classification_regime", _get("prior_regime", ""))),
        "schema_version": str(_get("schema_version", "")),
        "task_family": str(_get("task_family", "synthetic_nonlinear_classification_mixed_categorical")),
...
def build_index(session=None, expected_total: int | None = None) -> str:
    """Build META_NONLINEAR_MIXED_CLASSIFICATION_DATASET_INDEX."""
    if session is None:
    ...
    # The writer inserts 22 explicit columns; any DDL change must be reflected here.
    _EXPECTED_COL_COUNT = 22
    ...
    if len(_live_cols) != _EXPECTED_COL_COUNT:
        raise RuntimeError(
            ...
            "sql/nonlinear_classification_mixed_pipeline.sql before running this script."
        )
    ...
    return f"Indexed {rows_inserted} nonlinear mixed-classification tasks across {SPLITS}."
