"""Build META_CLASSIFICATION_DATASET_INDEX from staged classification parquet."""
...
_DATA_SUBDIR = "linear/classification/numeric"
INDEX_TABLE = "META_CLASSIFICATION_DATASET_INDEX"
SPLITS = ("train", "val", "test")
HPO_BUCKETS = 40
LOCAL_ROOT = "/tmp/meta_classification_dataset_index"


def _expected_total() -> int:
    return int(os.getenv("META_CLASSIFICATION_DATASET_EXPECTED_TOTAL", "1000"))
...
def _read_metadata(local_path: str) -> dict:
    ...
    if missing:
        raise ValueError(
            f"Classification parquet {local_path} is missing metadata: {sorted(missing)}"
        )
    table = pq.read_table(local_path, columns=sorted(required | (optional & available)))
    data = table.slice(0, 1).to_pydict()
    if table.num_rows < 1:
        raise ValueError(f"Classification parquet has no rows: {local_path}")
    ...
    # Warn loudly if p_signal/p_total are absent so a writer regression is not silently masked.
    # Under normal operation write_classification_parquet always emits these fields
    # (dgp_helpers.py:2715-2717); absence means the parquet was written by an older or broken
    # code path and the fallback values (p_signal=p, p_noise=0, p_total=p) will be incorrect
    # for any task that has genuine noise features.
    if "p_signal" not in available:
        print(
            f"[WARNING] build_meta_classification_dataset_index: parquet {local_path!r} is "
            f"missing p_signal/p_noise/p_total — falling back to p_signal=p={p}, p_noise=0, "
            "p_total=p.  This indicates a writer regression; re-generate the affected file.",
            flush=True,
        )
    return {
        "n": int(data["n"][0]),
        ...
        "p_signal": int(data.get("p_signal", [p])[0]),
        "p_noise": int(data.get("p_noise", [0])[0]),
        "p_total": int(data.get("p_total", [p])[0]),
...
def _write(session, rows: list[dict]) -> None:
    ...
    if len(live_cols) != len(expected_cols):
        raise RuntimeError(
            ...
            f"Re-deploy the DDL in sql/linear_classification_numeric_pipeline.sql before running this script. "
...
def _preflight_k_coverage(rows: list[dict]) -> None:
    ...
    if not result["coverage_passed"]:
        import warnings
        warnings.warn(
            f"[build_meta_classification_dataset_index] K coverage incomplete. "
...
def main() -> None:
    ...
    print(
        ...
        f"(META_CLASSIFICATION_DATASET_EXPECTED_TOTAL={_expected_total()})"
    )
