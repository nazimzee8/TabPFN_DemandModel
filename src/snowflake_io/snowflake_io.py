...
from task_routing import canonical_meta_subdir, get_training_data_spec
...
# Backward-compat alias — prefer META_NONLINEAR_REGRESSION_DATASET_INDEX going forward
...
def materialize_meta_dataset_stage(local_root="/tmp/data", splits=DEFAULT_SPLITS):
    ...
    Stage layout: @META_DATASET_STAGE/{linear|nonlinear}/{regression|classification}/{numeric|mixed}/{split}/
    The full subdir is derived from TRAINING_DATA_FAMILY via canonical_meta_subdir().
    """
    family = os.getenv("TRAINING_DATA_FAMILY", "") or "unknown"
    ...
    for split in splits:
        _subdir = canonical_meta_subdir(family, split)
        split_dir = os.path.join(local_root, split)
        os.makedirs(split_dir, exist_ok=True)
        session.file.get(f"{_stage}/{_subdir}/", split_dir)
        parquet_files = [
        ...
        if not parquet_files:
            raise FileNotFoundError(
                f"No .parquet files materialized for split {split!r} "
                f"from {_stage}/{_subdir}/ into {split_dir}"
            )
...
def _normalize_stage_path(stage_path, split=None):
    ...
    if split and str(split) not in path.split("/"):
