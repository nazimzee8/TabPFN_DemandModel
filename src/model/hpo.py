...
# Gate hidden dim — single fixed value; pretrain runs once at this width.
# fusion_gate_hidden_dim is pinned to 64 in all regression search spaces (not tuned).
GATE_HIDDEN_DIM_CANDIDATES = [64]

# MODEL4/5 runtime selectors — propagated to HPO workers via env vars.
...
# linear_model: tunes optimizer/regularization and MODEL4 head dims; architecture fixed.
# linear_model_architecture: tunes d_phi/n_sab_feat; cold-start allowed on mismatch.
# lbacnp_model: MODEL5 LBACNP search space — handles both linear and nonlinear families.
_HPO_SWEEP_MODE_ALIASES = {
    "ridge_residual": "linear_model",
    "architecture": "linear_model_architecture",
    "linear_stats": "linear_model",
    # deprecated nonlinear modes → lbacnp_model
    ...
    # deprecated aliases → lbacnp_model
...
_hpo_is_nonlinear = HPO_DATA_SPEC.is_nonlinear or HPO_SWEEP_MODE == "lbacnp_model"
DEFAULT_MAX_INDEX_P = "512" if _hpo_is_nonlinear else str(MODEL4_MAX_P)
DEFAULT_MAX_INDEX_N_TRAIN = "1024" if _hpo_is_nonlinear else str(MODEL4_MAX_N_TRAIN)
...
def build_hpo_search_space(tune, baseline_config=None) -> dict:
    ...
    linear_model (default):
        ...
        Tunes: lr, weight_decay, dropout, use_huber, huber_delta, lambda_l1, MODEL4 head dims and
    ...
    if HPO_SWEEP_MODE == "lbacnp_model":
        _is_nl = HPO_DATA_SPEC.is_nonlinear
        _is_cls = HPO_TASK_OBJECTIVE == CLASSIFICATION_OBJECTIVE
        _space = {
            "lr":                   tune.loguniform(1e-4, 3e-3),
            "weight_decay":         tune.loguniform(1e-6, 1e-3),
            ...
            # LBACNP core
            "use_lbacnp":           True,
            "lbacnp_latent_dim":    tune.choice([64, 128, 256]),
            "lbacnp_latent_max":    tune.choice([32, 64]),
            "lbacnp_latent_frac":   tune.choice([0.125, 0.25, 0.5]),
            "lbacnp_n_blocks":      tune.choice([2, 3, 4]),
            "lbacnp_heads":         4,
            "lbacnp_dropout":       tune.uniform(0.0, 0.2),
            "lbacnp_output_mode":   "last",
            # PLE
            "use_numeric_ple":      True if _is_nl else tune.choice([False, True]),
            "ple_num_bins":         tune.choice([16, 32, 64]) if not _is_cls else 32,
            "ple_boundary_mode":    "context_quantile",
            "ple_target_aware":     False,
            "ple_projection_dim":   tune.choice([64, 128]),
            # Distribution head
            "regression_distribution_head": (not _is_cls),
            "nll_loss_weight":      tune.choice([0.0, 0.1, 0.25]) if not _is_cls else 0.0,
            "mse_loss_weight":      1.0,
            "min_log_var":          -8.0,
            "max_log_var":          4.0,
            # Residual gate and penalty
            "linear_lbacnp_residual": (not _is_nl),
            "linear_lbacnp_residual_gate_init": tune.choice([-4.0, -3.0, -2.0]) if not _is_nl else -3.0,
            "lbacnp_residual_penalty_weight": tune.choice([0.0, 1e-5, 1e-4]) if not _is_nl else 0.0,
            # Linear stats backbone (used only for non-nonlinear families as residual substrate)
            "use_linear_stats":     (not _is_nl),
            "use_coefficient_head": (not _is_nl),
            "use_lambda_head":      (not _is_nl),
        }
        if not _is_nl and not _is_cls:
            _space.update({
                "linear_stat_dim":           tune.choice([64, 128]),
                "coeff_head_hidden_dim":     tune.choice([32, 64]),
                "lambda_head_hidden_dim":    tune.choice([16, 32]),
                "linear_encoder_hidden_dim": tune.choice([128, 256]),
                "fusion_gate_hidden_dim":    64,
                "beta_aux_loss_weight":      tune.loguniform(0.01, 0.5),
                "pred_aux_loss_weight":      tune.loguniform(0.005, 0.2),
        ...
        if _is_cls:
            _space.update({
                "use_classification_path": True,
                "use_class_label_embeddings": True,
                "use_class_coefficient_head": (not _is_nl),
                "use_class_bias_head": (not _is_nl),
                "use_class_residual_head": False,
                "use_class_stat_fusion": (not _is_nl),
                "use_class_stats": (not _is_nl),
                "use_classification_stats": (not _is_nl),
                "max_num_classes": 10,
                "class_embedding_dim": tune.choice([32, 64, 128]),
                "class_stat_dim": tune.choice([64, 128]) if not _is_nl else 64,
                "class_stat_hidden_dim": tune.choice([128, 256]) if not _is_nl else 128,
                "class_head_hidden_dim": tune.choice([32, 64]),
                "class_fusion_gate_hidden_dim": tune.choice([32, 64]) if not _is_nl else 32,
                "class_ce_loss_weight": 1.0,
                "class_logit_kl_loss_weight": tune.choice([0.0, 0.025, 0.05]),
                "class_coef_aux_loss_weight": tune.choice([0.0, 0.025]) if not _is_nl else 0.0,
                "class_margin_aux_loss_weight": tune.choice([0.0, 0.01]) if not _is_nl else 0.0,
                "class_prior_aux_loss_weight": tune.choice([0.0, 0.01]) if not _is_nl else 0.0,
                "class_calibration_aux_loss_weight": 0.0,
                "class_imbalance_reweighting": tune.choice([False, True]),
                "class_label_smoothing": tune.choice([0.0, 0.05]),
            })
    ...
    arch_space = {
        ...
        "use_linear_stats": bool(baseline_config.get("use_linear_stats", True)),
        "use_coefficient_head": bool(baseline_config.get("use_coefficient_head", True)),
        "use_lambda_head": bool(baseline_config.get("use_lambda_head", True)),
...
def _build_ray_trainable(hpo_data_ref, pretrain_ckpt_map_ref):
    ...
    def ray_trainable(config):
        ...
        # MODEL5-LBACNP fields
        use_lbacnp                       = bool(config.get("use_lbacnp", False))
        lbacnp_latent_dim                = int(config.get("lbacnp_latent_dim", 128))
        lbacnp_latent_max                = int(config.get("lbacnp_latent_max", 64))
        lbacnp_latent_frac               = float(config.get("lbacnp_latent_frac", 0.25))
        lbacnp_n_blocks                  = int(config.get("lbacnp_n_blocks", 4))
        lbacnp_heads                     = int(config.get("lbacnp_heads", 4))
        lbacnp_dropout                   = float(config.get("lbacnp_dropout", 0.1))
        lbacnp_output_mode               = str(config.get("lbacnp_output_mode", "last"))
        use_numeric_ple                  = bool(config.get("use_numeric_ple", False))
        ple_num_bins                     = int(config.get("ple_num_bins", 32))
        ple_boundary_mode                = str(config.get("ple_boundary_mode", "context_quantile"))
        ple_target_aware                 = bool(config.get("ple_target_aware", False))
        ple_projection_dim               = int(config.get("ple_projection_dim", 128))
        linear_lbacnp_residual           = bool(config.get("linear_lbacnp_residual", False))
        linear_lbacnp_residual_gate_init = float(config.get("linear_lbacnp_residual_gate_init", -3.0))
        lbacnp_residual_penalty_weight   = float(config.get("lbacnp_residual_penalty_weight", 0.0))
        regression_distribution_head     = bool(config.get("regression_distribution_head", False))
        nll_loss_weight                  = float(config.get("nll_loss_weight", 0.0))
        mse_loss_weight                  = float(config.get("mse_loss_weight", 1.0))
        min_log_var                      = float(config.get("min_log_var", -8.0))
        max_log_var                      = float(config.get("max_log_var", 4.0))
        ...
        pretrain_ckpt = None
        if use_lbacnp:
            pretrain_ckpt = pretrain_ckpt_map.get("lbacnp")
            if pretrain_ckpt:
                _saved_cfg = pretrain_ckpt.get("cfg")
                _arch_mismatches = checkpoint_architecture_mismatches(_saved_cfg, cfg)
                if _arch_mismatches:
                    print(
                        f"[HPO lbacnp] Pretrain architecture mismatch; cold-starting trial: "
                        f"{_arch_mismatches}",
                        flush=True,
                    )
                    pretrain_ckpt = None
            else:
                print("[HPO lbacnp] No lbacnp pretrain checkpoint; cold-starting.", flush=True)
...
def main():
    ...
    if HPO_SWEEP_MODE in ("linear_model", "linear_model_architecture"):
        ...
        # lbacnp_model sweep: populate "lbacnp" key so trials can warm-start from the
        # nonlinear pretrain checkpoint. Explicit path takes precedence; otherwise
        # auto-detect pretrain_nonlinear_meta.pt in @MODEL_STAGE/checkpoints/.
        checkpoint_map = {}
        _lbacnp_ckpt_path = HPO_PRETRAIN_CHECKPOINT_STAGE_PATH
        if not _lbacnp_ckpt_path:
            # Auto-detect: look for the canonical nonlinear pretrain checkpoint.
            _NONLINEAR_PRETRAIN_FILENAME = "pretrain_nonlinear_meta.pt"
            _lbacnp_ckpt_candidate = f"@MODEL_STAGE/checkpoints/{_NONLINEAR_PRETRAIN_FILENAME}"
            try:
                from snowflake.snowpark import Session as _Session
                _probe_session = _Session.builder.getOrCreate()
                _stage_files = {
                    str(r[0]).rstrip("/").rsplit("/", 1)[-1]
                    for r in _probe_session.sql("LIST @MODEL_STAGE/checkpoints/").collect()
                }
                if _NONLINEAR_PRETRAIN_FILENAME in _stage_files:
                    _lbacnp_ckpt_path = _lbacnp_ckpt_candidate
                    print(
                        f"[HPO lbacnp] Auto-detected nonlinear pretrain checkpoint: "
                        f"{_lbacnp_ckpt_path}",
                        flush=True,
                    )
                else:
                    print(
                        f"[HPO lbacnp] Nonlinear pretrain checkpoint not found "
                        f"({_NONLINEAR_PRETRAIN_FILENAME}); trials will cold-start. "
                        "Run CALL run_pretrain_pipeline_nonlinear(...) first.",
                        flush=True,
                    )
            except Exception as _probe_exc:
                print(
                    f"[HPO lbacnp] Could not probe @MODEL_STAGE/checkpoints/ "
                    f"for pretrain checkpoint: {_probe_exc}. Trials will cold-start.",
                    flush=True,
                )
        if _lbacnp_ckpt_path:
            # Register under "lbacnp" — the key that Ray trainable trials look up.
            checkpoint_map["lbacnp"] = _lbacnp_ckpt_path
            print(
                f"[HPO lbacnp] Pretrain checkpoint registered for warm-start: "
                f"{_lbacnp_ckpt_path}",
                flush=True,
            )
    ...
    if HPO_DATA_SPEC.is_nonlinear or HPO_SWEEP_MODE == "lbacnp_model":
        validate_nonlinear_hpo_coverage(hpo_rows)

    if HPO_SWEEP_MODE in ("linear_model", "lbacnp_model"):
    ...
    if HPO_SWEEP_MODE == "linear_model_architecture":
    ...
    best_config = {
        ...
        # Prefer the arch version embedded in the search-space result (e.g. "model5_lbacnp"
        # from the lbacnp_model search space at build_hpo_search_space line ~810).
        # Fall back to the module-level MODEL_ARCH_VERSION which is now propagated
        # correctly by run_hpo_job.py ("model5_lbacnp" for lbacnp_model sweeps, "model4"
        # otherwise).
        "model_arch_version":   best_config_raw.get("model_arch_version", MODEL_ARCH_VERSION),
        ...
        # MODEL5-LBACNP fields (populated for lbacnp_model sweep; defaults safe for MODEL4)
        "use_lbacnp":           bool(best_config_raw.get("use_lbacnp", False)),
        "lbacnp_latent_dim":    int(best_config_raw.get("lbacnp_latent_dim", 128)),
        "lbacnp_latent_max":    int(best_config_raw.get("lbacnp_latent_max", 64)),
        "lbacnp_latent_frac":   float(best_config_raw.get("lbacnp_latent_frac", 0.25)),
        "lbacnp_n_blocks":      int(best_config_raw.get("lbacnp_n_blocks", 4)),
        "lbacnp_heads":         int(best_config_raw.get("lbacnp_heads", 4)),
        "lbacnp_dropout":       float(best_config_raw.get("lbacnp_dropout", 0.1)),
        "lbacnp_output_mode":   str(best_config_raw.get("lbacnp_output_mode", "last")),
        "use_numeric_ple":      bool(best_config_raw.get("use_numeric_ple", False)),
        "ple_num_bins":         int(best_config_raw.get("ple_num_bins", 32)),
        "ple_boundary_mode":    str(best_config_raw.get("ple_boundary_mode", "context_quantile")),
        "ple_target_aware":     bool(best_config_raw.get("ple_target_aware", False)),
        "ple_projection_dim":   int(best_config_raw.get("ple_projection_dim", 128)),
        "linear_lbacnp_residual": bool(best_config_raw.get("linear_lbacnp_residual", False)),
        "linear_lbacnp_residual_gate_init": float(best_config_raw.get("linear_lbacnp_residual_gate_init", -3.0)),
        "lbacnp_residual_penalty_weight": float(best_config_raw.get("lbacnp_residual_penalty_weight", 0.0)),
        "regression_distribution_head": bool(best_config_raw.get("regression_distribution_head", False)),
        "nll_loss_weight":      float(best_config_raw.get("nll_loss_weight", 0.0)),
        "mse_loss_weight":      float(best_config_raw.get("mse_loss_weight", 1.0)),
        "min_log_var":          float(best_config_raw.get("min_log_var", -8.0)),
        "max_log_var":          float(best_config_raw.get("max_log_var", 4.0)),
        ...
        "_meta": {
            ...
            "pretrain_warm_start_policy": "cold_start_default_exact_match_optional",
            "pretrain_checkpoint_map": {
                str(k): path for k, path in checkpoint_map.items()
            },
            "pretrain_checkpoint_stage_path": checkpoint_map.get(
                "lbacnp"
                if bool(best_config_raw.get("use_lbacnp", False))
                else (
                    "classification"
                    if HPO_TASK_OBJECTIVE == CLASSIFICATION_OBJECTIVE
                    else "linear"
    ...
    if HPO_SWEEP_MODE == "linear_model_architecture" and baseline_config is not None:
