...
NONLINEAR_TRAINING_FAMILY = "synthetic_nonlinear_regression"

# All four nonlinear task families — used by is_nonlinear property.
# Includes the legacy alias "synthetic_regression_nonlinear" so that
# get_training_data_spec("synthetic_regression_nonlinear").is_nonlinear is True
# even when get_training_data_spec returns a new spec with family set to the alias string.
_NONLINEAR_FAMILIES: frozenset[str] = frozenset({
    NONLINEAR_TRAINING_FAMILY,
    "synthetic_regression_nonlinear",               # backward-compat alias for nonlinear-regression
    NONLINEAR_CLASSIFICATION_TRAINING_FAMILY,
    NONLINEAR_MIXED_REGRESSION_TRAINING_FAMILY,
    NONLINEAR_MIXED_CLASSIFICATION_TRAINING_FAMILY,
})
...
class TrainingDataSpec:
    ...
    def is_mixed(self) -> bool:
        return "mixed" in self.family

    @property
    def linearity_segment(self) -> str:
        return "nonlinear" if self.is_nonlinear else "linear"

    @property
    def objective_segment(self) -> str:
        return "classification" if self.is_classification else "regression"

    @property
    def feature_segment(self) -> str:
        return "mixed" if self.is_mixed else "numeric"

    @property
    def data_subdir(self) -> str:
        """Canonical stage subdir: {linear|nonlinear}/{regression|classification}/{numeric|mixed}"""
        return f"{self.linearity_segment}/{self.objective_segment}/{self.feature_segment}"


def canonical_meta_subdir(family: str, split: str) -> str:
    """Return the canonical META_DATASET_STAGE subdir for (family, split).

    Format: {linear|nonlinear}/{regression|classification}/{numeric|mixed}/{split}
    Example: "linear/regression/numeric/train"
    """
    spec = get_training_data_spec(family)
    return f"{spec.data_subdir}/{split}"


def canonical_eval_subdir(family: str) -> str:
    """Return the canonical EVALUATION_DATASET_STAGE subdir for a training family.

    Format: {linear|nonlinear}/{regression|classification}/{numeric|mixed}/eval
    Example: "nonlinear/classification/mixed/eval"
    """
    spec = get_training_data_spec(family)
    return f"{spec.data_subdir}/eval"
...
_LINEAR_REGRESSION_SPEC = TrainingDataSpec(
    ...
    index_builder="build_meta_regression_dataset_index",
)
...
_FAMILY_SPECS = {
    ...
    NONLINEAR_TRAINING_FAMILY: TrainingDataSpec(
        ...
        index_builder="build_meta_nonlinear_regression_dataset_index",
    ),
    # Back-compat alias for configs/checkpoints serialized with the old family name
    "synthetic_regression_nonlinear": TrainingDataSpec(
        family=NONLINEAR_TRAINING_FAMILY,
        task_objective=REGRESSION_OBJECTIVE,
        index_table="META_NONLINEAR_REGRESSION_DATASET_INDEX",
        stage=META_DATASET_STAGE,
