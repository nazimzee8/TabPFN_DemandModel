...
def _low_rank_compositional_teacher(rng: np.random.Generator, Xa: np.ndarray, params: dict) -> np.ndarray:
    ...
    k = Xa.shape[1]
    r = max(1, min(k, int(rng.integers(2, min(k, 8) + 1))))
...
def _sample_feature_params(
    ...
    *,
    ...
    if feature_regime == "iid_dense":
        ...
        # p_noise = 2–3 × p_signal irrelevant features
        multiplier = float(rng.choice([2.0, 2.5, 3.0]))
        p_noise = int(round(multiplier * p_signal))
        noise_feature_fraction = float(p_noise) / float(p_signal + p_noise)
        return {
            "covariance_type": "iid",
            "rho": 0.0,
            "active_fraction": 1.0,
...
def write_parquet(ds: dict, filepath: str) -> None:
    ...
    Preserves the schema expected by build_meta_nonlinear_dataset_index.py.
...
def main() -> None:
    ...
    for idx in range(n_datasets):
        ...
        n = int(np.random.default_rng(base_seed + idx).choice(V2_N_GRID))
        p_signal = int(np.random.default_rng(base_seed + idx + 1).choice(V2_P_SIGNAL_GRID))
        noise_scale = float(np.random.default_rng(base_seed + idx + 2).choice(V2_TARGET_NOISE_SCALES))
        noise_type = str(np.random.default_rng(base_seed + idx + 3).choice(V2_TARGET_NOISE_TYPES))

        teacher_seed = int(np.random.default_rng([base_seed, idx, 0]).integers(2**62))
        sample_seed = int(np.random.default_rng([base_seed, idx, 1]).integers(2**62))
        ...
        try:
            ...
            print(f"  [{idx:4d}] LearnabilityGateError — skipping {target_family}/{feature_regime}")
            continue
        ...
        if (idx + 1) % 100 == 0:
            print(f"  [{idx + 1:4d}/{n_datasets}] {filename} -> {split_dir}")
...
def write_parquet_training_mixed_regression(ds: dict, filepath: str) -> None:
    ...
    Includes X_cat for the categorical columns.
    """
    X = ds["X"]
    X_cat = ds["X_cat"]
    y = ds["y"]
    ...
    while generated < n_datasets and attempts < max_attempts:
        ...
        try:
            if task_family == "synthetic_nonlinear_classification":
                ...
                p_cat = int(np.random.default_rng(base_seed ^ seed_magic ^ idx ^ 4).choice(_MIXED_P_CAT_GRID))
                ...
                class_imbalance_type = str(np.random.default_rng(base_seed ^ seed_magic ^ idx ^ 5).choice(_CLS_IMBALANCE_GRID))
                margin_level = str(np.random.default_rng(base_seed ^ seed_magic ^ idx ^ 6).choice(_CLS_MARGIN_GRID))
                p_cat = int(np.random.default_rng(base_seed ^ seed_magic ^ idx ^ 7).choice(_MIXED_P_CAT_GRID))
