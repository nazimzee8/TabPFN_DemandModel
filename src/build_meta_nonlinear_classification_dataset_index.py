...
tasks. Reads from @META_NONLINEAR_CLASSIFICATION_DATASET_STAGE/{split}/ and writes to
...
def _list_files(session, split: str) -> list[str]:
    rows = session.sql(f"LIST {STAGE}/{split}/").collect()
    return sorted(
...
def _download(session, split: str, task_id: str) -> str:
    ...
    session.file.get(f"{STAGE}/{split}/{filename}", local_dir)
    if os.path.exists(local_path):
        return local_path
    candidates = sorted(
