...
_IS_MIXED = os.getenv("SYNREG_IS_MIXED_CATEGORICAL", "").lower() in ("1", "true", "yes")
# SYNREG_STAGE_PREFIX: datasets are loaded via stage_path round-tripped from the index table, so
# this constant is not used by the main eval loops. Kept for completeness and spot-check use.
# Append /eval to match the prep writer stage structure.
SYNREG_STAGE_PREFIX = os.getenv(
    "SYNREG_STAGE_PREFIX",
    "@EVALUATION_DATASET_STAGE/linear/regression/mixed/eval"
    if _IS_MIXED
    else "@EVALUATION_DATASET_STAGE/linear/regression/numeric/eval",
)
...
SYNREG_RESULTS_STAGE = os.getenv(
    ...
    (
        ...
        if EVAL_IS_CLASSIFICATION
            ...
            "@EVALUATION_RESULTS_STAGE/linear/regression/mixed"
            if _IS_MIXED
...
# Fallback to legacy names for backward compat with previously trained checkpoints.
...
CHECKPOINT_STAGE_PATH = _FAMILY_QUALIFIED_CKPT  # preferred; legacy: best_regression.pt
