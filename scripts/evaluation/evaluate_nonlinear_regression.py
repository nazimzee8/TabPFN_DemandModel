"""
evaluate_nonlinear_regression.py
==================================
Nonlinear synthetic-regression evaluation engine (standalone fork of
evaluate_linear_regression.py with nonlinear-specific defaults).
...
SYNREG_SUITE_ID = os.getenv("SYNTHETIC_REGRESSION_SUITE_ID", "nonlinear")
NONLINEAR_SUITE_ID = SYNREG_SUITE_ID
...
SYNREG_RESULTS_STAGE = os.getenv(
    ...
    (
        "@EVALUATION_RESULTS_STAGE/nonlinear/classification/numeric"
        if EVAL_IS_CLASSIFICATION
        else "@EVALUATION_RESULTS_STAGE/nonlinear/regression/numeric"
    ),
)
...
SYNREG_INDEX_TABLE = os.getenv(
    ...
    (
        ...
        if EVAL_IS_CLASSIFICATION
        else "NONLINEAR_REGRESSION_DATASET_INDEX"
    ),
)
...
# Family-qualified checkpoint name (written by run_model_training_job.py M4 fix).
# In real runs SYNREG_DEEPSET_CHECKPOINT_STAGE_PATH is always set by the orchestrator.
_FAMILY_QUALIFIED_CKPT = (
    f"@MODEL_STAGE/checkpoints/best_{EVAL_DATA_SPEC.data_subdir.replace('/', '_')}.pt"
)
CHECKPOINT_STAGE_PATH = _FAMILY_QUALIFIED_CKPT
...
SYNREG_NONLINEAR_BASELINES = os.getenv("SYNREG_NONLINEAR_BASELINES", "true").lower() in ("1", "true", "yes")
...
def _stage_path_to_local_name(stage_path: str) -> str:
    ...
    @EVALUATION_DATASET_STAGE/linear/regression/numeric/primary/dataset_0000.parquet
    → linear__regression__numeric__primary__dataset_0000.parquet
    """
...
def main(session=None) -> str | None:
    # If a session is provided, run orchestration (index build) mode
    if session is not None:
        return _run_orchestration(session)

    mode = SYNREG_MODE.lower()
    print(f"[INFO] evaluate_nonlinear_regression starting. MODE={mode} "
...
def _run_orchestration(session) -> str:
    """Orchestrate nonlinear v2 index build: migrate schema, truncate, insert rows."""
    ...
    # Download manifest from stage
    tmp_dir = tempfile.mkdtemp()
    session.file.get(
        f"@EVALUATION_DATASET_STAGE/nonlinear/{NONLINEAR_SUITE_ID}/manifest.json",
        tmp_dir,
    )
    ...
    for row in rows:
        ...
        session.sql(
            f"INSERT INTO {_NONLINEAR_INDEX_TABLE} ({cols}) VALUES ({vals})"
        ).collect()

    count = _assert_nonlinear_index_populated(session, len(rows))
    return f"OK: inserted {count} rows into {_NONLINEAR_INDEX_TABLE}"


# ---------------------------------------------------------------------------
# Nonlinear v2 index management helpers
# ---------------------------------------------------------------------------

_NONLINEAR_INDEX_TABLE = "NONLINEAR_REGRESSION_DATASET_INDEX"

_NONLINEAR_V2_MIGRATION_COLUMNS = [
...
_EXPECTED_TARGET_FAMILIES = [
    "poly_quad", "sin_low", "hinge", "sparse_interact", "mixed_linear", "demand_mono",
]


def _truncate_nonlinear_index(session) -> None:
    """Delete rows for the current suite_id from the nonlinear index."""
    session.sql(
        f"DELETE FROM {_NONLINEAR_INDEX_TABLE} WHERE suite_id = '{NONLINEAR_SUITE_ID}'"
    ).collect()


def _run_schema_migration(session) -> None:
    """Add v2 columns to the nonlinear index if they don't already exist."""
    for col_name, col_type in _NONLINEAR_V2_MIGRATION_COLUMNS:
        session.sql(
            f"ALTER TABLE {_NONLINEAR_INDEX_TABLE} "
            f"ADD COLUMN IF NOT EXISTS {col_name} {col_type}"
        ).collect()


def _build_index_rows(manifest: dict) -> list[dict]:
    """Convert a nonlinear manifest dict into index rows."""
    rows = []
    for ds in manifest.get("datasets", []):
        _n_total = int(ds.get("n_total", 0))
        rows.append({
            "logical_dataset_key": ds["logical_dataset_key"],
            "condition_id": ds.get("condition_id", ""),
            "feature_regime": ds.get("feature_regime", ""),
            "suite_component": ds.get("suite_component", "core"),
            "dataset_id": ds.get("dataset_idx", 0),
            "stage_path": ds.get("filename", ""),
            "n_total": _n_total,
            "n_train_default": round(_n_total * 0.8),
            "n_holdout_default": round(_n_total * 0.1),
            "p_signal": ds.get("p_signal", 0),
            "p_noise": ds.get("p_noise", 0),
            "p_total": ds.get("p_total", 0),
            "target_noise_scale": ds.get("target_noise_scale", 0.0),
            "target_noise_type": ds.get("target_noise_type", "gaussian"),
            "feature_noise_sigma": ds.get("feature_noise_sigma", 0.0),
            "normalization_constant": ds.get("normalization_constant", 1.0),
            "teacher_seed": ds.get("teacher_seed", 0),
            "sample_seed": ds.get("sample_seed", 0),
            "snr_target": ds.get("snr_target", 1.0),
            "rho": ds.get("rho", 0.0),
            "active_fraction": ds.get("active_fraction", 1.0),
            "covariance_type": ds.get("covariance_type", "iid"),
            "noise_feature_fraction": ds.get("noise_feature_fraction", 0.0),
            "prior_name": manifest.get("prior_name", "nonlinear"),
            "prior_version": manifest.get("prior_version", "v2"),
            "suite_id": manifest.get("suite_id", NONLINEAR_SUITE_ID),
            "suite_family": (
                "nonlinear_mixed_regression" if SYNREG_IS_MIXED_CAT
                else "nonlinear_regression"
            ),
            "source_suite_id": manifest.get("suite_id", manifest.get("prior_name", "")),
            "prior_regime": manifest.get("prior_name", ""),
        })
    return rows


def _validate_manifest_coverage(manifest: dict) -> None:
    """Raise ValueError if any expected target family is missing from the manifest."""
    families_present = {d["target_family"] for d in manifest.get("datasets", [])}
    for fam in _EXPECTED_TARGET_FAMILIES:
        if fam not in families_present:
            raise ValueError(
                f"Missing target family {fam!r} in manifest. "
                f"Present families: {sorted(families_present)}"
            )
...
def _validate_no_duplicate_keys(manifest: dict) -> None:
    ...
    seen = set()
    for ds in manifest.get("datasets", []):
        key = ds["logical_dataset_key"]
        if key in seen:
            raise ValueError(f"Duplicate logical_dataset_key: {key!r}")
        seen.add(key)


def _validate_stage_spot_check(session, manifest, n_spot=5) -> None:
    """Spot-check that a few stage paths are accessible."""
    pass


def _assert_nonlinear_index_populated(session, expected: int) -> int:
    """Check that the nonlinear index has the expected number of rows."""
    rows = session.sql(
        f"SELECT COUNT(*) FROM {_NONLINEAR_INDEX_TABLE} "
        f"WHERE suite_id = '{NONLINEAR_SUITE_ID}'"
    ).collect()
    actual = int(rows[0][0]) if rows else 0
    if actual < expected:
        raise RuntimeError(
            f"Expected at least {expected} rows in {_NONLINEAR_INDEX_TABLE}, "
            f"got {actual}"
        )
    return actual
