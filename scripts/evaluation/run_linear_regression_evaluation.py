"""
run_linear_regression_evaluation.py
========================================
Orchestrator for the split-phase linear regression evaluation suite.
...
def run_synthetic_regression_prep(
    ...
    job_prod = _submit_synreg(
        ...
        env_vars={
            ...
            # Propagate mixed-categorical routing flags — _set_regression_linear_env has already
            # set these in the current process; the prep SPCS job needs them too so that
            # prepare_linear_regression.py routes to LINEAR_MIXED_REGRESSION_DATASET_INDEX and
            # the mixed eval stage prefix when is_mixed_categorical=True.
            "SYNREG_IS_MIXED_CATEGORICAL": os.getenv("SYNREG_IS_MIXED_CATEGORICAL", "false"),
            "SYNREG_INDEX_TABLE": os.getenv(
                "SYNREG_INDEX_TABLE", "LINEAR_REGRESSION_DATASET_INDEX"
            ),
        },
        runtime_environment=prep_runtime_environment,
        entrypoint="prepare_linear_regression.py",
        target_instances=1,
        pip_requirements=None,
        external_access_integrations=None,
    )
...
def _resolve_regression_linear_checkpoint(session) -> str:
    """Return the deepset checkpoint path for linear regression (numeric or mixed).

    Resolution order:
      1. SYNREG_DEEPSET_CHECKPOINT_STAGE_PATH env var (explicit override).
      2. Family-qualified checkpoint: best_{data_subdir.replace('/',_')}.pt
         (written by run_model_training_job.py M4 fix).
      3. Legacy name: best_regression.pt (backward compat for previously trained models).
    """
    ...
    _family = os.environ.get("TRAINING_DATA_FAMILY", "synthetic_linear_regression")
    _spec = get_training_data_spec(_family)
    family_qualified = f"@MODEL_STAGE/checkpoints/best_{_spec.data_subdir.replace('/', '_')}.pt"

    for candidate in (family_qualified, "@MODEL_STAGE/checkpoints/best_regression.pt"):
        try:
            rows = session.sql(f"LIST {candidate}").collect()
            if rows:
                return candidate
        except Exception:
            pass
    ...
    raise RuntimeError(
        f"No checkpoint found. Tried: {family_qualified!r} and "
        "'@MODEL_STAGE/checkpoints/best_regression.pt'. "
        "Upload the checkpoint or set SYNREG_DEEPSET_CHECKPOINT_STAGE_PATH explicitly."
    )
...
def run_linear_regression_prep(
    session, is_mixed_categorical: bool, prep_rt: str, bench_rt: str, ag_rt: str,
...
def run_linear_regression_deepset_evaluation(
    session, is_mixed_categorical: bool, prep_rt: str, bench_rt: str, ag_rt: str,
    suite_id: str = "linear_all_v1",
) -> str:
...
def run_linear_regression_baseline_evaluation(
    session, is_mixed_categorical: bool, prep_rt: str, bench_rt: str, ag_rt: str,
    baseline_shards=None, baseline_concurrent_nodes=None,
) -> str:
    _set_regression_linear_env("linear_all_v1", is_mixed_categorical=is_mixed_categorical)
    return run_synthetic_regression_baseline_evaluation(
        session,
        bench_rt,
        bench_rt,
...
def run_linear_regression_autogluon_evaluation(
    session, is_mixed_categorical: bool, prep_rt: str, bench_rt: str, ag_rt: str,
    autogluon_concurrent_nodes=None,
    suite_id: str = "linear_all_v1",
    autogluon_cluster_shards=None, autogluon_workers_per_shard=None,
    autogluon_task_cpus=None, autogluon_time_limit=None, autogluon_presets=None,
) -> str:
    _set_regression_linear_env(suite_id, is_mixed_categorical=is_mixed_categorical)
...
def run_linear_regression_aggregation(
    session, is_mixed_categorical: bool, prep_rt: str, bench_rt: str, ag_rt: str,
    suite_id: str = "linear_all_v1",
...
def run_linear_regression_pipeline(
    ...
    results = []
    results.append(run_linear_regression_prep(
        session, is_mixed_categorical, prep_rt, bench_rt, ag_rt,
        suite_id=suite_id))
    results.append(run_linear_regression_deepset_evaluation(
        session, is_mixed_categorical, bench_rt, bench_rt, bench_rt,
        suite_id=suite_id))
    results.append(run_linear_regression_baseline_evaluation(
        session, is_mixed_categorical, bench_rt, bench_rt, bench_rt,
        baseline_shards, baseline_concurrent_nodes))
    results.append(run_linear_regression_autogluon_evaluation(
        session, is_mixed_categorical, bench_rt, bench_rt, ag_rt,
        ...
        autogluon_presets=autogluon_presets,
        suite_id=suite_id))
    results.append(run_linear_regression_aggregation(
        session, is_mixed_categorical, bench_rt, bench_rt, ag_rt,
        suite_id=suite_id))
    return "\n".join(results)
