"""
run_linear_classification_evaluation.py
==========================================
Snowflake stored-procedure handler orchestrator for the linear classification
evaluation suite.

Mirrors run_linear_regression_evaluation.py patterns for classification.
All public handler functions are registered as Snowflake stored procedure
HANDLER targets in sql/linear_classification_*_pipeline.sql.
...
Handler functions (SQL HANDLER targets):
  run_linear_classification_prep
  run_linear_classification_deepset_evaluation
  run_linear_classification_baseline_evaluation
  run_linear_classification_autogluon_evaluation
  run_linear_classification_aggregation
  run_linear_classification_pipeline

Each handler accepts IS_MIXED_CATEGORICAL BOOLEAN as its first SQL parameter,
routing numeric (FALSE) vs mixed-categorical (TRUE) evaluation variants.
"""
...
def run_linear_classification_prep(
    session,
    ...
    job = _submit_cls_job(
        ...
        entrypoint="prepare_linear_classification.py",
        target_instances=1,
    )
    _wait_done(job, label, session)
    return f"run_linear_classification_prep: ok suite_id={suite_id}"


def run_linear_classification_deepset_evaluation(
    session,
    is_mixed_categorical: bool,
    prep_rt: str,
    bench_rt: str,
    ...
    for i in range(deepset_shards):
        ...
        jobs.append((label, job))
    _wait_job_group(jobs, session)
    return f"run_linear_classification_deepset_evaluation: ok shards={deepset_shards}"


def run_linear_classification_baseline_evaluation(
    session,
    is_mixed_categorical: bool,
    prep_rt: str,
    bench_rt: str,
    ...
    return (
        f"run_linear_classification_baseline_evaluation: ok "
        f"shards={shard_count}"
    )


def run_linear_classification_autogluon_evaluation(
    session,
    is_mixed_categorical: bool,
    prep_rt: str,
    bench_rt: str,
    ag_rt: str,
    autogluon_concurrent_nodes: int = 60,
    suite_id: str = SYNCLS_DEFAULT_SUITE_ID,
    ...
    concurrent_units = _resolve_int(autogluon_concurrent_clusters, autogluon_concurrent_nodes)
    ...
    _wait_job_group(jobs, session)
    return (
        f"run_linear_classification_autogluon_evaluation: ok "
        f"shards={concurrent_units} mode=single_node"
    )


def run_linear_classification_aggregation(
    session,
    is_mixed_categorical: bool,
    prep_rt: str,
    bench_rt: str,
    ag_rt: str,
    suite_id: str = SYNCLS_DEFAULT_SUITE_ID,
    expected_ag_shards: int = 60,  # matches run_linear_classification_autogluon_evaluation default (60 shards)
    ...
    _wait_done(job, label, session)
    return (
        f"run_linear_classification_aggregation: ok suite_id={suite_id} "
...
def run_linear_classification_pipeline(
    session,
    is_mixed_categorical: bool,
    prep_rt: str,
    bench_rt: str,
    ...
    # Resolve AG shard count ONCE here so the same value is passed to both the AG-eval job
    # (which submits that many shards) and aggregation (which expects that many files).
    # Default 60 matches run_linear_classification_autogluon_evaluation's autogluon_concurrent_nodes=60.
    concurrent_units = _resolve_int(autogluon_concurrent_clusters, 60)
    results = []
    results.append(
        run_linear_classification_prep(
            session, is_mixed_categorical, prep_rt, bench_rt, ag_rt,
            suite_id=suite_id,
        )
    )
    results.append(
    ...
    results.append(
        run_linear_classification_autogluon_evaluation(
            session, is_mixed_categorical, prep_rt, bench_rt, ag_rt,
            suite_id=suite_id,
            autogluon_concurrent_nodes=concurrent_units,  # explicit — eliminates the 60-vs-10 mismatch
    ...
    results.append(
        run_linear_classification_aggregation(
            session, is_mixed_categorical, prep_rt, bench_rt, ag_rt,
            suite_id=suite_id,
            expected_ag_shards=concurrent_units,  # same source of truth
...
def main() -> None:
    ...
    parser.add_argument("--mixed", action="store_true", default=False,
                        help="Route to mixed-categorical suite (IS_MIXED_CATEGORICAL=True)")
    ...
    phase_map = {
        "prep":                   lambda: run_linear_classification_prep(
                                      session, args.mixed, args.prep_rt, args.bench_rt, args.ag_rt,
                                      suite_id=args.suite_id),
        "deepset_evaluation":     lambda: run_linear_classification_deepset_evaluation(
                                      session, args.mixed, args.prep_rt, args.bench_rt, args.ag_rt,
                                      suite_id=args.suite_id),
        "baseline_evaluation":    lambda: run_linear_classification_baseline_evaluation(
                                      session, args.mixed, args.prep_rt, args.bench_rt, args.ag_rt,
                                      suite_id=args.suite_id),
        "autogluon_evaluation":   lambda: run_linear_classification_autogluon_evaluation(
                                      session, args.mixed, args.prep_rt, args.bench_rt, args.ag_rt,
                                      suite_id=args.suite_id),
        "aggregation":            lambda: run_linear_classification_aggregation(
                                      session, args.mixed, args.prep_rt, args.bench_rt, args.ag_rt,
                                      suite_id=args.suite_id),
        "pipeline":               lambda: run_linear_classification_pipeline(
                                      session, args.mixed, args.prep_rt, args.bench_rt, args.ag_rt,
                                      suite_id=args.suite_id),
    }
