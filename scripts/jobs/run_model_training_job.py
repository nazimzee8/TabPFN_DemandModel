...
_HPO_SWEEP_MODE_ALIASES = {
    # Canonical linear aliases
    "ridge_residual": "linear_model",
    "architecture": "linear_model_architecture",
    "linear_stats": "linear_model",
    # All deprecated nonlinear/lbacnp aliases → lbacnp_model (unified with run_hpo_job.py)
...
def _validate_nonlinear_training_index(session) -> None:
    """Pre-flight for META_NONLINEAR_REGRESSION_DATASET_INDEX + @META_DATASET_STAGE/nonlinear/regression/."""
    try:
        rows = session.sql(
            "SELECT split, COUNT(*) AS task_count "
    ...
    for _split in ("train", "val"):
        try:
            ...
            if not _files:
                raise RuntimeError(
                    f"No staged files in @META_DATASET_STAGE/nonlinear/regression/numeric/{_split}/. "
            ...
            raise RuntimeError(
                f"@META_DATASET_STAGE/nonlinear/regression/numeric/{_split}/ inaccessible: {exc}"
            ) from exc
...
def _validate_nonlinear_index_by_spec(session, spec) -> None:
    ...
    for _split in ("train", "val"):
        try:
            # Use spec.data_subdir so the path is e.g.
            # @META_DATASET_STAGE/nonlinear/classification/mixed/train/
            # rather than the bare @META_DATASET_STAGE/train/ (which is always empty).
            _split_path = f"{stage}/{spec.data_subdir}/{_split}/"
            files = session.sql(f"LIST {_split_path}").collect()
            if not files:
                raise RuntimeError(
                    f"No staged files found in {_split_path}. "
                    "Re-upload training data before starting a GPU job."
                )
        except RuntimeError:
            raise
        except Exception as exc:
            raise RuntimeError(f"{stage}/{spec.data_subdir}/{_split}/ inaccessible: {exc}") from exc
...
def _run_model_training_impl(
    ...
    # Authoritative nonlinear detection: covers the canonical lbacnp_model sweep mode (written by
    # hpo.py post-D1 fix), the spec's is_nonlinear flag, and any legacy use_lbacnp/
    # use_latent_ridge_expert keys that older configs may carry.
    is_nonlinear_config = (
        get_training_data_spec(training_data_family).is_nonlinear
        or hpo_sweep_mode == "lbacnp_model"
        or bool(best_config.get("use_lbacnp"))
        or bool(best_config.get("use_latent_ridge_expert"))
    )
    pretrain_policy = (
    ...
    if _meta_ckpt:
        ...
        # Gate pretrain is collapsed to gate=64 only (L2); train.py reads fusion_gate_hidden_dim
        # from BEST_CONFIG, not from GATE_HIDDEN_DIM env var, so all linear runs use gate64.
        _gate_ckpt_name = (
        ...
        if not _stage_file_exists(session, f"{MODEL_STAGE}/checkpoints/", _gate_ckpt_name):
            raise FileNotFoundError(
                f"[run_model_training] No pretrain checkpoint found. "
                f"Expected: {_gate_ckpt_path} in @MODEL_STAGE/checkpoints/. "
                "Run the gate pretrain first: "
                "CALL run_pretrain_pipeline('...', '...', '...', 64);"
            )
    ...
    # Family-qualified checkpoint names prevent numeric and mixed from overwriting each other
    # at the same @MODEL_STAGE/checkpoints/ path.
    # Pattern: best_{data_subdir.replace('/','_')}.pt
    # e.g.  linear/regression/numeric → best_linear_regression_numeric.pt
    #        nonlinear/classification/mixed → best_nonlinear_classification_mixed.pt
    _ckpt_spec = get_training_data_spec(training_data_family)
    checkpoint_name = f"best_{_ckpt_spec.data_subdir.replace('/', '_')}.pt"
    ...
    env_vars = {
        ...
        # Note: META_*_EXPECTED_TOTAL env vars below are NOT read by train.py (L3 cleanup).
        # Index validation is done handler-side before submit_from_stage(); they are kept here
        # only for diagnostic transparency (visible in job env dumps) but are otherwise inert.
    }
...
def run_model_training_model(
    ...
    MODEL_ARCH_VERSION is not a parameter here — train.py reads model_arch_version from
    best_config.json (written by HPO) via hyper_params.get("model_arch_version", MODEL_ARCH_VERSION),
    falling back to the MODEL_ARCH_VERSION env var (default "model4") if absent.
    For lbacnp pipelines, best_config.json contains "model_arch_version": "model5_lbacnp"
    (set by run_hpo_job.py → hpo.py). Do not override this via SQL SET or a separate env var.
