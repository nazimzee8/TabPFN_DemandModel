...
# SQL HANDLER alias: both numeric + mixed regression SQL procs bind this name
build_meta_regression_dataset_index_with_flag_and_total = build_meta_dataset_index_with_flag_and_total
...
def _validate_meta_dataset_index(session):
    ...
    for _split in ("train", "val"):
        try:
            _files = session.sql(
                f"LIST @META_DATASET_STAGE/linear/regression/numeric/{_split}/"
            ).collect()
            if not _files:
                raise RuntimeError(
            ...
            raise RuntimeError(
                ...
                f"@META_DATASET_STAGE/linear/regression/numeric/{_split}/ "
...
def _validate_nonlinear_training_index(session) -> None:
    ...
    for _split in ("train", "val"):
        try:
            _files = session.sql(
                f"LIST @META_DATASET_STAGE/nonlinear/regression/numeric/{_split}/"
            ).collect()
            if not _files:
                raise RuntimeError(
                    f"No staged files in "
                    f"@META_DATASET_STAGE/nonlinear/regression/numeric/{_split}/. "
