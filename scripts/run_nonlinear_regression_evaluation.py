...
Stored procedure handlers (all callable from SQL):
  ...
  run_nonlinear_regression_autogluon_spcs_evaluation (4-arg and 9-arg overloads)
  run_nonlinear_regression_aggregation               (1-arg and 4-arg overloads)
...
def run_nonlinear_regression_baseline_evaluation_default(
    session,
    bench_rt: str = "2.5.0-py311",
) -> str:
    """SQL handler: 1-arg overload (bench_rt only)."""
    return run_nonlinear_regression_baseline_evaluation(session, bench_rt)


def run_nonlinear_regression_baseline_evaluation_with_shards(
    session,
    bench_rt: str = "2.5.0-py311",
    baseline_shards: int = SYNREG_CPU_SHARDS,
) -> str:
    """SQL handler: 2-arg overload (bench_rt, baseline_shards)."""
    return run_nonlinear_regression_baseline_evaluation(
        session, bench_rt, baseline_shards=baseline_shards
    )


def run_nonlinear_regression_baseline_evaluation_with_shards_and_concurrency(
    session,
    bench_rt: str = "2.5.0-py311",
    baseline_shards: int = SYNREG_CPU_SHARDS,
...
def run_nonlinear_regression_autogluon_spcs_evaluation(
    session,
    ag_rt: str = "spcs_job",
    ...
    _sp  = _nonlinear_regression_suite_params(is_mixed_categorical)
    proc = "run_nonlinear_regression_autogluon_spcs_evaluation"
...
def run_nonlinear_regression_autogluon_spcs_evaluation_default(
    session,
    ag_image: str,
    ...
    return run_nonlinear_regression_autogluon_spcs_evaluation(
        session,
        ag_rt=ag_image,
...
def run_nonlinear_regression_autogluon_spcs_evaluation_full(
    session,
    ag_image: str,
...
def run_nonlinear_regression_aggregation_default(
    ...
    return run_nonlinear_regression_aggregation(session, bench_rt)
...
# ---------------------------------------------------------------------------
# Mixed-categorical nonlinear regression — SQL-callable wrappers
# All 5 phases delegate to the parameterized core handlers with is_mixed_categorical=True.
# ---------------------------------------------------------------------------

def run_nonlinear_mixed_regression_prep(
    session,
    bench_rt: str = "2.5.0-py311",
) -> str:
    """Phase 1 (mixed): Populate NONLINEAR_MIXED_REGRESSION_DATASET_INDEX."""
    return run_nonlinear_regression_prep(session, bench_rt, is_mixed_categorical=True)


def run_nonlinear_mixed_regression_deepset_evaluation(
    session,
    bench_rt: str = "2.5.0-py311",
) -> str:
    """Phase 2 (mixed): DeepSet GPU evaluation against the mixed-categorical eval index."""
    return run_nonlinear_regression_deepset_evaluation(
        session, bench_rt, is_mixed_categorical=True
    )


def run_nonlinear_mixed_regression_baseline_evaluation(
    session,
    bench_rt: str = "2.5.0-py311",
) -> str:
    """Phase 3 (mixed): Baseline CPU evaluation against the mixed-categorical eval index."""
    return run_nonlinear_regression_baseline_evaluation(
        session, bench_rt, is_mixed_categorical=True
    )


def run_nonlinear_mixed_regression_baseline_evaluation_with_shards(
    session,
    bench_rt: str = "2.5.0-py311",
    baseline_shards: int = SYNREG_CPU_SHARDS,
) -> str:
...
def run_nonlinear_mixed_regression_baseline_evaluation_with_shards_and_concurrency(
    session,
    bench_rt: str = "2.5.0-py311",
    baseline_shards: int = SYNREG_CPU_SHARDS,
    baseline_concurrent_nodes: int = SYNREG_CPU_SHARDS,
) -> str:
    """Phase 3 (mixed, 3-arg overload): baseline with shards + concurrency."""
    return run_nonlinear_regression_baseline_evaluation(
        session, bench_rt,
        baseline_shards=baseline_shards,
        baseline_concurrent_nodes=baseline_concurrent_nodes,
...
def run_nonlinear_mixed_regression_autogluon_spcs_evaluation(
    session,
    ag_image: str,
    autogluon_cluster_shards: int = 0,
    autogluon_workers_per_shard: int = 1,
    ...
    """Phase 4 (mixed): AutoGluon SPCS evaluation against the mixed-categorical eval index."""
    return run_nonlinear_regression_autogluon_spcs_evaluation(
        session,
        ag_rt=ag_image,
        autogluon_cluster_shards=autogluon_cluster_shards,
...
def run_nonlinear_mixed_regression_autogluon_spcs_evaluation_full(
    session,
    ag_image: str,
    autogluon_cluster_shards: int,
    autogluon_workers_per_shard: int,
    ...
    """Phase 4 (mixed, 9-arg overload): AutoGluon SPCS full parameters."""
    return run_nonlinear_regression_autogluon_spcs_evaluation(
        session,
        ag_rt=ag_image,
        autogluon_cluster_shards=autogluon_cluster_shards,
...
def run_nonlinear_mixed_regression_aggregation(
    session,
    bench_rt: str = "2.5.0-py311",
) -> str:
    """Phase 5 (mixed): Aggregate mixed-categorical nonlinear regression shards."""
    return run_nonlinear_regression_aggregation(session, bench_rt, is_mixed_categorical=True)


def run_nonlinear_mixed_regression_aggregation_full(
    session,
    bench_rt: str,
    expected_deepset_shards: int,
    expected_baseline_shards: int,
