...
-----
    ...
    python scripts/generation/generate_nonlinear_regression.py --n-datasets 420 --out-dir data/nonlinear_regression/numeric

    # v3 suite (17 families, ~630 datasets):
    python scripts/generation/generate_nonlinear_regression.py --v3 --n-datasets 630 --out-dir data/nonlinear_regression/numeric

    # v3 mixed-categorical regression:
    python scripts/generation/generate_nonlinear_regression.py --v3 --mixed-categorical --n-datasets 100

    # Smoke test:
    python scripts/generation/generate_nonlinear_regression.py --v3 --n-datasets 5 --dry-run

    # Generate core component only:
    python scripts/generation/generate_nonlinear_regression.py --v3 --suite-component core

Output
------
    v2:        data/nonlinear_regression/numeric/eval/{family}__dataset_{idx:05d}.parquet
    v3:        data/nonlinear_regression/numeric/eval/{family}__dataset_{idx:05d}.parquet
    v3 mixed:  data/nonlinear_regression/mixed/eval/{family}__dataset_{idx:05d}.parquet
"""
...
def _generate_and_write_v2(
    ...
    filename = f"{target_family}__dataset_{idx:05d}.parquet"
...
def _generate_and_write_v3(
    ...
    *,
    ...
    if is_mixed_categorical:
        ...
        entry["categorical_cardinalities"] = [int(c) for c in ds.get("categorical_cardinalities", [])]
        # p_num = numeric feature count = p_signal + p_noise (= p_total for numeric features)
        entry["p_num"] = int(entry.get("p_signal", 0)) + int(entry.get("p_noise", 0))
    return entry
...
def main() -> None:
    ...
    if args.v3:
        if args.mixed_categorical:
            default_out = "data/nonlinear_regression/mixed"
            default_n = 100
            manifest_filename = "nonlinear_mixed_regression_v1_manifest.json"
            suite_id = "nonlinear_mixed_regression"
            stage_prefix = "@EVALUATION_DATASET_STAGE/nonlinear/regression/mixed/eval"
            prior_version = "mixed_v1"
            target_families_list = V3_TARGET_FAMILIES
        else:
            default_out = "data/nonlinear_regression/numeric"
            default_n = 630
            manifest_filename = "nonlinear_v3_manifest.json"
            suite_id = "nonlinear_v3"
            stage_prefix = "@EVALUATION_DATASET_STAGE/nonlinear/regression/numeric/eval"
            prior_version = "v3"
            target_families_list = V3_TARGET_FAMILIES
    else:
        default_out = "data/nonlinear_regression/numeric"
    ...
    manifest_path = eval_dir / manifest_filename
    ...
    # SnowSQL PUT commands — flat eval layout: one PUT for all parquets, one for manifest
    abs_eval_dir = eval_dir.resolve().as_posix()
    abs_manifest = manifest_path.resolve().as_posix()
    print("\n--- SnowSQL PUT commands ---")
    print(
        f"PUT file://{abs_eval_dir}/*.parquet "
        f"{stage_prefix}/ "
