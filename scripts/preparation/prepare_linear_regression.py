...
SYNREG_STAGE_PREFIX = "@EVALUATION_DATASET_STAGE/linear/regression/numeric"
...
def insert_synreg_index_rows(session, rows: list[dict]) -> None:
    ...
    col_list = (
        ...
        "task_fingerprint, base_task_id, perturbation_condition, task_seed, "
        "training_data_family, task_objective"
    )

    # Runtime schema alignment guard: validate col_list against TABLE_COLUMN_REGISTRY
    # so column drift between this writer and the DDL is caught at job start, not
    # at Snowflake SQL execution time.
    try:
        from schema_contract import validate_row_against_registry
        _sample_row = {c.strip(): None for c in col_list.split(",")}
        validate_row_against_registry(_sample_row, SYNREG_INDEX_TABLE.upper(), strict=True)
        print(f"[INFO] Schema alignment validated: col_list matches {SYNREG_INDEX_TABLE}.", flush=True)
    except ImportError:
        pass  # schema_contract not on path in this environment; skip runtime guard
...
def prepare_primary_suite(rng: np.random.Generator, local_dir: str, session) -> list[dict]:
    ...
    stage_dir = f"{SYNREG_STAGE_PREFIX}/eval"
...
def _build_mixed_reg_index_record(
    ...
    if "p_signal" not in meta:
        warnings.warn(
            f"p_signal missing from entry (dataset_id={dataset_id}), defaulting to p_num={p_num}",
            stacklevel=2,
        )
    return {
        "suite_id": suite_id,
...
def _prepare_mixed_regression(session) -> str:
    ...
    stage_prefix = "@EVALUATION_DATASET_STAGE/linear/regression/mixed/eval"
