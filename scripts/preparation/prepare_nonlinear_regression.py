...
Purpose
-------
Downloads nonlinear_v2_manifest.json from the hierarchical evaluation stage path
(@EVALUATION_DATASET_STAGE/nonlinear/regression/numeric/ for numeric datasets,
@EVALUATION_DATASET_STAGE/nonlinear/regression/mixed/ for mixed-categorical),
...
# Route stage prefix based on whether this run targets mixed-categorical datasets.
# Set SYNREG_IS_MIXED_CATEGORICAL=true (or 1) for the mixed suite; leave unset for numeric.
_IS_MIXED = os.environ.get("SYNREG_IS_MIXED_CATEGORICAL", "false").lower() in ("true", "1")
if _IS_MIXED:
    _EVAL_STAGE_PREFIX = "@EVALUATION_DATASET_STAGE/nonlinear/regression/mixed/eval"
else:
    _EVAL_STAGE_PREFIX = "@EVALUATION_DATASET_STAGE/nonlinear/regression/numeric/eval"

NONLINEAR_MANIFEST_PATH = f"{_EVAL_STAGE_PREFIX}/nonlinear_v2_manifest.json"
...
# 13 new metadata columns added by schema migration, plus 4 mixed-categorical columns
_V2_NEW_COLUMNS = [
    "feature_regime",
    "covariance_type",
    "rho",
...
def _run_schema_migration(session) -> None:
    ...
    migrations = [
        ...
        f"ALTER TABLE {NONLINEAR_INDEX_TABLE} ADD COLUMN IF NOT EXISTS p_num                     NUMBER",
        f"ALTER TABLE {NONLINEAR_INDEX_TABLE} ADD COLUMN IF NOT EXISTS p_cat                     NUMBER",
        f"ALTER TABLE {NONLINEAR_INDEX_TABLE} ADD COLUMN IF NOT EXISTS categorical_cardinalities ARRAY",
        f"ALTER TABLE {NONLINEAR_INDEX_TABLE} ADD COLUMN IF NOT EXISTS max_cardinality           NUMBER",
    ]
...
def _insert_nonlinear_rows(session, rows: list[dict]) -> None:
    ...
    try:
        ...
        validate_row_against_registry(_sample_row, NONLINEAR_INDEX_TABLE.upper(), strict=True)
        print(f"[INFO] Schema alignment validated: col_list matches {NONLINEAR_INDEX_TABLE}.", flush=True)
    except ImportError:
        pass
