"""
prepare_linear_classification.py
=====================================
Classification equivalent of prepare_linear_regression.py.

Reads parquet files from
  @EVALUATION_DATASET_STAGE/linear/classification/numeric/<suite_id>/  (numeric)
  @EVALUATION_DATASET_STAGE/linear/classification/mixed/<suite_id>/    (mixed-categorical)
...
SYNCLS_STAGE_PREFIX = "@EVALUATION_DATASET_STAGE/linear/classification/numeric"
...
# Canonical stage root: generator writes to linear/classification/numeric/{suite_id}/{family}/
SYNCLS_STAGE_ROOT = "@EVALUATION_DATASET_STAGE/linear/classification/numeric"
...
def _insert_cls_index_rows(session, rows: list[dict]) -> None:
    ...
    col_list = (
        ...
        "hidden_holdout_suite_id, task_fingerprint, task_seed, coeff_schema_version"
    )
    ...
    for r in rows:
        ...
        select_strings.append(
            ...
            f"{_sql_num(r.get('task_seed', r.get('teacher_seed', 0)))}, "
            f"{_sql_str(r.get('coeff_schema_version', r.get('schema_version', '')))}"
        )
    ...
    try:
        ...
        validate_row_against_registry(_sample_row, SYNCLS_INDEX_TABLE.upper(), strict=True)
        print(f"[INFO] Schema alignment validated: col_list matches {SYNCLS_INDEX_TABLE}.", flush=True)
    except ImportError:
        pass
...
def _build_index_record(
    ...
    classification_regime = meta.get("classification_regime") or meta.get("regime") or meta.get("prior_regime")
...
def _insert_mixed_cls_index_rows(session, rows: list[dict]) -> None:
    ...
    # Runtime schema alignment guard (mixed-cat path).
    try:
        from schema_contract import validate_row_against_registry
        _sample_row = {c.strip(): None for c in col_list.split(",")}
        validate_row_against_registry(_sample_row, SYNCLS_INDEX_TABLE.upper(), strict=True)
...
def _prepare_mixed_classification(session) -> str:
    ...
    stage_prefix = f"@EVALUATION_DATASET_STAGE/linear/classification/mixed/eval"
...
def prepare_synthetic_classification(session=None) -> str:
    ...
    # Discover parquet files on stage (canonical path: linear/classification/numeric/eval)
    stage_prefix = f"{SYNCLS_STAGE_ROOT}/eval"
    parquet_paths = _list_stage_parquets(session, stage_prefix)
    if not parquet_paths:
        raise FileNotFoundError(
            f"No parquet files found under {stage_prefix}. "
