...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Output goes to mixed_regression_prepared/ instead of synthetic_regression_prepared/."
        ),
    )
...
def _run_mixed_regression(args: argparse.Namespace) -> dict[str, Any]:
    ...
    Output goes to mixed_regression_prepared/{suite_id}/{regime}/ with
    stage path @EVALUATION_DATASET_STAGE/mixed_regression_prepared/...
    """
    if args.n_datasets < 1:
        raise ValueError("--n_datasets must be >= 1")
    suite_dir = Path(args.out_dir) / "mixed_regression_prepared" / args.suite_id
    if args.dry_run:
    ...
    for idx, assignment in enumerate(assignments):
        ...
        stage_path = (
            f"@EVALUATION_DATASET_STAGE/mixed_regression_prepared/"
            f"{args.suite_id}/{regime}/{filename}"
        )
        records.append({
            "dataset_id":           idx,
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    suite_dir = Path(args.out_dir) / "synthetic_regression_prepared" / args.suite_id
    ...
    for family in enabled:
        ...
        for family_idx, assignment in enumerate(assignments):
            ...
            filename = f"dataset_{family_idx:04d}.parquet"
            ...
            record = {
                ...
                "suite_family": family,
                "stage_path": (
                    f"@EVALUATION_DATASET_STAGE/synthetic_regression_prepared/"
                    f"{args.suite_id}/{family}/{filename}"
                ),
                "prior_regime": regime,
    ...
    manifest_path = suite_dir / "synthetic_regression_manifest.json"
    manifest_path.write_text(json.dumps(manifest, indent=2), encoding="utf-8")
...
def main() -> None:
    ...
    if manifest.get("dry_run_allocation"):
        return
    subdir = "mixed_regression_prepared" if getattr(args, "mixed_categorical", False) else "synthetic_regression_prepared"
    print(
        f"Generated {len(manifest['datasets'])} datasets under "
        f"{Path(args.out_dir) / subdir / args.suite_id}"
    )
