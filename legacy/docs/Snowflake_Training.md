...
All generation commands are run from the repo root (`C:/Documents/TabPFN_DemandModel`).
Each orchestrator hard-codes the canonical flags (n_datasets, profile, seed) so you
rarely need to call the underlying engines directly.

**Seed convention**: linear scripts use `--base_seed`; nonlinear scripts use `--seed`.

---

#### Training data — linear (numeric)

Produces 1 000 datasets split 80/10/10 into `data/linear_regression/numeric/{train,val,test}/`
and `data/linear_classification/numeric/{train,val,test}/`.

```bash
# Both families (regression + classification):
python scripts/generation/run_generate_linear_numeric.py

# One family only:
python scripts/generation/run_generate_linear_numeric.py --only regression
python scripts/generation/run_generate_linear_numeric.py --only classification
```

Internally calls `src/data_generation/generate_dgp.py` with:
- Regression: `--task_family linear_regression --n_datasets 1000 --profile linear_stat_aware --base_seed 42 --store_beta --store_teacher_preds`
- Classification: `--task_family linear_classification --n_datasets 1000 --profile linear_classification_stat_aware --base_seed 42`

---

#### Training data — linear (mixed-categorical)

Produces 1 000 datasets split 80/10/10 into `data/linear_regression/mixed/{train,val,test}/`
and `data/linear_classification/mixed/{train,val,test}/`. The coexisting `numeric/` subdir is
left intact (only `mixed/` is cleaned before each run).
...
Internally calls `src/data_generation/generate_dgp.py` with:
- Mixed regression: `--task_family linear_regression_mixed_categorical --n_datasets 1000 --profile linear_regression_mixed_categorical_stat_aware --base_seed 42`
- Mixed classification: `--task_family linear_classification_mixed_categorical --n_datasets 1000 --profile linear_classification_mixed_categorical_stat_aware --base_seed 42`

---

#### Training data — nonlinear (numeric)

Produces 1 000 datasets split 80/10/10 into `data/nonlinear_regression/numeric/{train,val,test}/`
and `data/nonlinear_classification/numeric/{train,val,test}/`. Each `out_dir` is fully deleted
before regeneration.
...
Internally calls `src/data_generation/generate_nonlinear_dgp.py` with:
- Regression: `--task_family synthetic_nonlinear_regression --n_datasets 1000 --out_dir data/nonlinear_regression/numeric --seed 42`
- Classification: `--task_family synthetic_nonlinear_classification --n_datasets 1000 --out_dir data/nonlinear_classification/numeric --seed 42`

---

#### Training data — nonlinear (mixed-categorical)

Produces 1 000 datasets split 80/10/10 into `data/nonlinear_regression/mixed/{train,val,test}/`
and `data/nonlinear_classification/mixed/{train,val,test}/`.
...
Internally calls `src/data_generation/generate_nonlinear_dgp.py` with:
- Mixed regression: `--task_family synthetic_nonlinear_regression_mixed_categorical --n_datasets 1000 --out_dir data/nonlinear_regression/mixed --seed 42`
- Mixed classification: `--task_family synthetic_nonlinear_classification_mixed_categorical --n_datasets 1000 --out_dir data/nonlinear_classification/mixed --seed 42`

---

#### Evaluation suites (separate from training data)

Eval suites write flat files under `eval/` subdirs, not `{train,val,test}/`. All eval
scripts support `--dry-run` to print what would be generated without writing files.

**Linear regression eval suite** (200 datasets × 11 suite families, numeric + optional mixed):

```bash
python scripts/generation/generate_linear_regression.py \
    --n_datasets 200 \
    --profile linear_stat_aware \
    --base_seed 20260512 \
    --out_dir data \
    --suite_id linear_regression_eval_v2
```

Add `--mixed_categorical` to generate mixed-categorical regression eval instead.
Env var `SYNREG_IS_MIXED_CATEGORICAL=1` triggers the same switch for orchestrator use.

**Linear classification eval suite**:

```bash
python scripts/generation/generate_linear_classification.py \
    --n_datasets 1000 --n_datasets_per_sweep 50 \
    --suite_id linear_classification_v1 \
    --base_seed 20260512 --store_class_params
```

Add `--mixed_categorical` for mixed-categorical classification eval.
Env var `SYNCLS_IS_MIXED_CATEGORICAL=1` triggers the same switch.

**Nonlinear regression eval suite** (v3, 630 datasets across 17 families):

```bash
python scripts/generation/generate_nonlinear_regression.py --v3 --n-datasets 630
```

v2 backward-compat (420 datasets, 6 families): omit `--v3`.
Mixed-categorical v3: add `--mixed-categorical`.
Subset: `--suite-component core|robustness|ood` (default `all`).

**Nonlinear classification eval suite** (v3, 400 datasets across 11 families):

```bash
python scripts/generation/generate_nonlinear_classification.py --n-datasets 400
```

Mixed-categorical classification eval: add `--mixed-categorical`.
Subset: `--suite-component core|robustness|ood` (default `all`).

---

#### Underlying engines (direct invocation — rarely needed)

**Linear engine** (`src/data_generation/generate_dgp.py`):

```bash
python src/data_generation/generate_dgp.py \
...
Supported `--task_family` values: `linear_regression`, `linear_classification`,
`linear_regression_mixed_categorical`, `linear_classification_mixed_categorical`.

**Nonlinear engine** (`src/data_generation/generate_nonlinear_dgp.py`):

```bash
python src/data_generation/generate_nonlinear_dgp.py \
...
Supported `--task_family` values: `synthetic_nonlinear_regression`,
`synthetic_nonlinear_classification`, `synthetic_nonlinear_regression_mixed_categorical`,
